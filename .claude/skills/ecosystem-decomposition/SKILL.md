...
- `docs/fringeisland-thinking/bible--fringeisland-universe.md` — the Universe Bible, the world-building truth that Vision references as structural concepts: the worlds (chapter 2), the beings and the Whisp (chapter 3), the roles (chapter 6); its glossary (chapter 9) is the vocabulary authority. The folder's `README.md` is the index. Consult for naming and conceptual alignment.
...
- **The payload walk.** When a platform contract spec is written ahead of its surface consumer, the decomposition session walks the consuming stories against the proposed payload shape before either spec is declared `4-ready`: every field a story renders or mutates traces to a named key in the proposed payload, and every proposed key to a consumer. The walk covers **every payload the surface renders from** — including *other* features' payloads the new surface consumes, not just the cycle's own (the J-D rider: the Progress panel keyed on `enrollment_id`, which the sibling feature's summary payload never served). Misses found at build are decomposition faults, not build riders (the `get_journey_detail` lesson; the J-D `enrollment_id` rider). **The walk traces keys — it does not see rendered copy.** Any acceptance criterion that *quotes* a user-facing string is verified against what the surface will actually render, not against the key that feeds it: N-E's H042 STORY-1 said the chip "shows 'Accepted'" while the correct render (per the N-B answerer-row precedent) reads "Accepted by [own nickname]", and the payload walk passed it because the *key* was right. Quote-bearing ACs get a copy check in the same pass.

- **The mechanism walk — read what you are leaning on.** The payload walk traces *keys*; this traces *behaviour*. **For every existing mechanism a spec depends on being true, read it and cite the evidence — `file:line` or a catalogue query — before `4-ready`.** Not "we believe retire allows this"; the query, in the spec. The type specimen is RD-C, where **three** premises in one `4-ready` Core spec were false and **none was caught by review**: (1) the delete guard rested on *"publication rows are never deleted"* — `admin_unpublish_role_template` hard-deletes them, so a template that *was* offered would have read as never-offered and been destroyed, breaching RD-4a in exactly the case RD-4 protects; (2) *"retire audits its refusals"*, copied into a new story as the posture to match — measured, **0 rows matching `%_refused` out of 6 619**, because the family `RAISE`s in the same transaction that wrote the audit row, which Postgres then discards; (3) an AC requiring a refusal *"surfaced verbatim"* — the guard raised `42501`, which the BFF maps to an existence-hiding 404, so the reason never reached the admin. Every one was a single catalogue query or `grep` away at spec time; every one instead cost a build-time correction, and two were caught only because a test happened to assert them. **The shape to watch for: a spec sentence that describes what some *other*, already-shipped thing does.** That sentence is a claim about the world, not a design decision, and it is checkable now.
...
- **Which conformance gates does this object class face?** When a story introduces an object of a kind the codebase has not carried before — a first-of-its-kind trigger mount, ownership pairing, table class, or function/table owner combination — the decomposition names the **structural** conformance gates it will meet before the spec goes `4-ready`, not just the assertion-level siblings a keyword sweep finds. N-E is the type specimen: the spec-time sweep enumerated sibling assertions thoroughly and still missed GC-8, the cross-owner trigger-mount gate, because a DS-5 function mounted on a PC-3 table was the first of its kind — the gate refused it at build and the license (citing ADR-U048 + U051 Amendment 2) had to be written under time pressure. The ownership-manifest registration rule is the same lesson at a smaller scale; this generalises it. A sweep finds what resembles the past; this question asks what the object is.
