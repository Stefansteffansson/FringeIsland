...
| **After a discovery session is baked into the Universe Bible, or an ADR sourced from universe-discovery is added** | Section 10 (Bible completeness) — every "Baked into the bible" item and open thread of the session has its home in the bible |
...
| **Three Worlds cosmology** (added 2026-06-10) | Ordinary World / Safe Harbour / The Other Side as the cosmological frame; the "three-dimensional void" | Superseded by the worlds chapter of the Universe Bible (`docs/fringeisland-thinking/bible--fringeisland-universe.md`): Ordinary World -> Shimmer -> Fringe (Nalome + Marath, each with a near side, a far side and a beyond); the Void as the axis of separation | "Three Worlds", "Safe Harbour" (as a world), "The Other Side", "void dimensions" |
...
| **Shadow as the anonymous entrant** (added 2026-06-21) | The pre-signup anonymous identity state was named the **Shadow** (ADR-U027) | Renamed the **Mist** (ADR-U031, discovery S47-48); ADR-U027 superseded by ADR-U031. **"Shadow" reassigned to the place-3 / sleep-paralysis menace** (the bible's worlds chapter). Two senses now: entrant-sense "Shadow" is drift → Mist; menace-sense "Shadow" (place 3) is correct. **Classify each hit by sense; never blind-replace.** Lifecycle cross-refs also moved ADR-U027 → ADR-U031 | `Shadow` (entrant sense = drift → Mist; place-3-menace sense = correct), and `U027` (lifecycle cross-refs → U031) |
...
| **The 15-minute own-edit/delete window** (added 2026-08-21) | Forum own-post edit and delete were bounded by a fixed 15-minute window (FEAT-PD011 CB-3 / FEAT-H028 COM-12: contract-side refusal, client-side affordance ticker, dedicated "window has closed" copy) | Retired by TASK-EDT-01 (edit ruled 2026-08-19, delete 2026-08-21): both acts are **unlimited**; transparency replaced the clock — "(edited)" renders when `updated_at − created_at > 3 minutes` (the silent typo-repair grace). **Classify by sense:** describing the window as history (spec Implementation notes, C-D cycle records, the amendment notes themselves) is correct; presenting a window as a *current* bound — a refusal copy, an expiring affordance, "within 15 minutes" as a live rule — is drift. The wielded no-edit posture (PD019) is a different rule and untouched | "15-minute", "edit window", "window has closed", `EDIT_WINDOW` (as a current mechanism; historical/amendment mentions are fine) |
...
| **The canon cores, their planned pages and the graduation tracker** (added 2026-09-26) | Seven `canon--*` files in `docs/fringeisland-thinking/` as the universe's truth, each with a Status grade and "Planned — not yet written" pages; the graduation tracker in the discovery file's Part 1; the ratification pass | Superseded by the one Universe Bible (`docs/fringeisland-thinking/bible--fringeisland-universe.md`) at the switch-over on 2026-09-26: settled facts are its chapters, open questions its Open boxes indexed in chapter 8, vocabulary its glossary; a session's "Baked into the bible" list is carried across by Claude Code. The canon files were deleted (git keeps them), the tracker retired, the two Phase 2 records deleted | "cosmology core", "roles core", "beings core", "narrative core", "growth core", "community core", "canonical core", "canon--", "graduation tracker", "Planned — not yet written", "ratification pass" (as a live step) — a hit is a finding only in an active file that presents them as a place to read or a step to take; historical files keep them |
...
| **"The admin family audits its refusals"** (added 2026-08-10) | Every `admin_*` guard wrote a `%_refused` row into `admin_audit_log` immediately before its `RAISE`, and FEAT-PC027 / FEAT-PC028 both stated in their Observability vertical that refusals "are recorded as refusals" | Retired by `TASK-RDC-03`: the `INSERT` sat in the transaction its own `RAISE` aborted, so Postgres discarded it — **0 rows matching `%_refused` out of 6 808**, across 46 distinct actions, since those functions shipped. **Ruled option 1** (migration `20260810150000`): the dead INSERTs are **deleted** and refusal traceability is a **deliberate non-goal**, said so in both specs. **Classify by sense:** *"refusals surface to the caller verbatim and are never swallowed"* remains TRUE and is **not** drift — only a claim that the **audit trail** records refusals is. **Retired in the same pass: a business refusal raised as `42501`.** `hub/lib/admin/roles.ts` collapses every `42501` into the admin-plane existence-hiding 404, so a *rule* refusal reached the admin as "Not found" about a template they could see. Business refusals are `P0001` → 409 verbatim; `42501` is the **non-admin gate only** — a `42501` assertion on that gate is correct and must not be swept |
| **"A new public table inherits its Data API grants from the default ACL"** (added 2026-09-23) | Supabase's `postgres` default ACL handed every new `public` table `SELECT` for `anon`/`authenticated` and `ALL` for `service_role` at CREATE time; 40 of 42 tables got their grants that way and no migration wrote them; TASK-SEC-02 left SELECT and the `service_role` set to it on purpose | Retired by Supabase on **2026-10-30** (the notice of 2026-09; TASK-SEC-03): from that date a table without its own GRANT lines is unreachable for every role. The house block, migration checklist row 8 and the platform-tier rule state the grants per table in the creating migration; `20260923120000` states them for the 42 existing tables. **Classify by sense:** the *function* default ACL (TASK-SEC-01 — Postgres's built-in `EXECUTE TO PUBLIC` on top of the row) is a different mechanism and is NOT retired; describing what the table default ACL *did* is historical and correct; a doc, code comment or house pattern that relies on a new table *inheriting* its table grants is drift | "default ACL" / "default privileges" said of TABLE grants as a current mechanism (function-grant hits are correct as-is) | `audits its refusals`, `recorded as refusals`, `refusal row`, `audit-logged on success and refusal`; and `42501` asserted for a **business** (non-permission) refusal |
...
| The `canvas--*` register (7 files) of `docs/fringeisland-thinking/` | TASK-UNI-03 (2026-09-15) | `docs/fringeisland-thinking/canvas--*` (before that, `docs/ecosystem/thinking/canvas-experiment/`) | Four app-side remnants moved to the **`FringeIsland-canvas` repo** (`design/mockups/phase-1/feedback-v0.4.md`, `design/mockups/phase-1/canvas-dark-VARIANT.png`, `docs/2026-09-14_tab-taxonomy-at-revert.md`, `docs/2026-09-14_what-was-kept-at-revert.md`; commit `15921fb`); **`canvas--the-seam.pdf` → `docs/novel/the-seam.pdf`** (it is a short story); the two Gimbal images **deleted** — the app serves them from its data folder, which holds byte-identical copies, and the canvas repo refuses to commit them by policy. The register no longer exists (five remain). **Grep note:** the 2026-09-14 bridge names `canvas--what-was-kept.md` — historical, correct |

| The five canon scaffolds of `docs/fringeisland-thinking/` — `canon--beings--whisp.md`, `canon--beings--npcs.md`, `canon--narrative--seasons-and-episodes.md`, `canon--narrative--journeys.md`, `canon--narrative--first-experience.md` | TASK-UNI-04 ruling D1 (2026-09-16) | `docs/fringeisland-thinking/` (created 2026-09-15 by TASK-UNI-03, empty by design) | Each was a **"Planned — not yet written"** entry inside its core — `canon--beings--whisp-and-npcs.md` and `canon--narrative--how-story-works.md`, both deleted 2026-09-26 — and is today an Open box in the Universe Bible (the Whisp's specification B-01; NPC authoring B-17 to B-19; seasons, episodes and journeys S-07; the first hour S-14) |
| `docs/fringeisland-thinking/canon--community--member-archetypes.md` | TASK-UNI-04 rulings D2 + D3 (2026-09-16) | `docs/fringeisland-thinking/` (before that `docs/ecosystem/universe/community/`) | The section "The member archetypes" of `canon--growth--engagement-spectrum.md` (Thinking grade carried on the section); since pass 4 the same day (TASK-UNI-05) that file was itself merged into `canon--growth--how-growth-works.md`, deleted 2026-09-26; the archetypes are the bible's Open box G-03 |
| `docs/fringeisland-thinking/discovery--2026-05-18--the-universe-explained.md` (the Session 01 onboarding summary) | TASK-UNI-04 ruling D4 (2026-09-16) | `docs/fringeisland-thinking/` (before that `docs/ecosystem/thinking/universe-discovery/2026-05-18_universe-discovery-onboarding-summary.md`) | None needed — pre-Mist and superseded: the three Canonical cores + the folder index carry its claims, Session 01 carries its influences; the read-through is in TASK-UNI-04 |
| `research--growth--what-fills-a-life-v2.{md,docx}` and `research--method--multi-product-ecosystem-management-rev2.docx` (renamed, not deleted) | TASK-UNI-04 item 10 (2026-09-16) | `docs/fringeisland-thinking/` | The same files without the version suffix — `research--growth--what-fills-a-life.{md,docx}`, `research--method--multi-product-ecosystem-management.docx` (v1 / rev1 retired the day before, so the suffix pointed at nothing) |

| `docs/fringeisland-thinking/canon--growth--three-questions.md` (moved) and `canon--growth--engagement-spectrum.md` (deleted) | TASK-UNI-05 pass 4 (2026-09-16, Stefan's list) | `docs/fringeisland-thinking/` | Both became `canon--growth--how-growth-works.md` (the three-questions file's history followed it by `git mv`), itself deleted 2026-09-26; the content is the bible's growth chapter |
| `docs/fringeisland-thinking/discovery--tracker-and-backlog.md`, `discovery--2026-05-28--portal-ideas-from-research.md`, `discovery--2026-07-24--gimbal-origin-and-altered-states.md` (deleted) and `discovery--2026-05-18--universe-session-01.md` (moved) | TASK-UNI-05 pass 4 (2026-09-16) | `docs/fringeisland-thinking/` | All four are **`discovery--the-universe-in-the-making.md`**, the one discovery file: Part 1 the tracker (headings "Discovery backlog (open topics awaiting sessions)" and "Graduation tracker" kept verbatim, one level deeper; `sources.json` and §10 repointed), Part 2 the two candidate notes, Part 3 the session record (its history follows the file). Headings inside the absorbed texts sit one or two levels deeper; no anchor links pointed at them |
| The seven canon files of `docs/fringeisland-thinking/` — `canon--cosmology--worlds-topology.md`, `canon--roles--taxonomy.md`, `canon--beings--whisp-and-npcs.md`, `canon--narrative--how-story-works.md`, `canon--growth--how-growth-works.md`, `canon--growth--privacy-model.md`, `canon--community--kickstarter-season-zero.md` | The Universe Bible switch-over (2026-09-26; Stefan chose deletion over pointer stubs) | `docs/fringeisland-thinking/` | **`docs/fringeisland-thinking/bible--fringeisland-universe.md`**, chapter by chapter: cosmology → 2 The worlds; roles → 6 Roles and governance; beings → 3 The beings; narrative → 5 Story; growth and the privacy model → 4 Growth; the Kickstarter → 7 Community and the founding moment. Active links were repointed at the switch-over; historical files (session bridges, retros, reference snapshots, archived openers, the TASK-UNI records, the superseded ADR-U027, `ARCHITECTURE_ANATOMY_V1.md`) keep the old paths by design |
| `docs/fringeisland-thinking/record--universe-bible-phase-2-rulings.md` and `record--universe-bible-phase-2-chapter-maps.md` | The Universe Bible switch-over (2026-09-26) | `docs/fringeisland-thinking/` | The bible itself: Stefan's rulings are in discovery Sessions 04–05 (Part 3), their outcomes in the bible; the chapter maps' statement-level provenance is in git history. Mentions in the sessions' "Relates to" lines are historical |
| `docs/fringeisland-thinking/discovery--2026-06-15--knowledge-base-whisp-and-universe-foundations.md` (moved across registers) | TASK-UNI-05 pass 4 (2026-09-16) | `docs/fringeisland-thinking/discovery--` | `research--growth--thinkers-and-models-behind-the-whisp.md` — by content a study with implications, the shape of the growth reports; text unchanged |
| `docs/fringeisland-thinking/research--engineering--performance-budget.md` (moved) and `research--engineering--cold-start-industry.md` (deleted) | TASK-UNI-05 pass 4 (2026-09-16) | `docs/fringeisland-thinking/` | Both are `research--engineering--performance-budget-and-cold-start.md`, Report A and Report B; ADR-U043 and ADR-U036 links repointed (path wording only) |
...
| Admin role-template contract bodies (`admin_retire_` / `admin_publish_` / `admin_delete_role_template`) | the **live catalogue** — `pg_proc` / `pg_get_functiondef`, never the migration text | `docs/planning/hub-v2/2026-08-06-rd-b-substrate-dossier.md` (banner'd 2026-08-10) |
...
- **The front door is current** (added 2026-09-05, COR-E W8 — Audit V R-14): `docs/planning/cycles/cycle-current.md` names a plan whose `**Status:**` is not closed, and its bridge link points at the newest file under `docs/planning/sessions/`. The unit gate `hub/tests/unit/platform/cycle-current-front-door.test.ts` is the mechanical half (fields, links, closed-status); the newest-bridge check is this run's by hand — a front door pointing at a bridge three sessions old is a finding, not a nit.
...
## Section 10 — Bible completeness (the discovery sessions and the Universe Bible)

**Question:** Has every discovery session been baked into the Universe Bible — every settled statement stated as true, every open thread an Open box in the index, every retired name in the glossary — and does every discovery-sourced ADR agree with the bible's vocabulary?

Universe-discovery sessions (Part 3 of `docs/fringeisland-thinking/discovery--the-universe-in-the-making.md`, the one discovery file) are working notes, never truth on their own. The truth is the one Universe Bible, `docs/fringeisland-thinking/bible--fringeisland-universe.md`: settled facts in chapters 1–7, open questions as Open boxes indexed in chapter 8, vocabulary in chapter 9. A session closes with a "Baked into the bible" list and an "Open threads" list; Claude Code carries them into the bible. (This section replaced the graduation-tracker check on 2026-09-26, when the canon files were deleted and the tracker retired.)

### Procedure

1. List the sessions in Part 3 (`grep -n "^### Session" docs/fringeisland-thinking/discovery--the-universe-in-the-making.md`) and the bible's Open boxes (`grep -c '^> <a id="open-' docs/fringeisland-thinking/bible--fringeisland-universe.md`).
2. **Forward check:** for each session closed since the last run, read its "Baked into the bible" and "Open threads" blocks; assert each item is either stated in a chapter or stands as an Open box (chapter 8 lists every box). A settled item with no sentence, or an open thread with no box, is a finding.
3. **Reverse check:** for each Open box, confirm no later session has answered it (search the box's subject in Part 3 after the session that raised it). An answered box that still stands is stale: replace it with the answer and remove its index line.
4. **ADR agreement:** for each discovery-sourced ADR (`grep -rlniE "universe-discovery|2026-06-05 product lock|session 0" docs/architecture/decisions/`), confirm its vocabulary agrees with the glossary (the far side, never "the Beyond"; the Mist; becoming a FIM as consent and transcendence as the birth; lines, not branches, for the bonds between FIMs; Nalome and Marath). ADRs bind: where an ADR and the bible differ on substance the bible owes a correction; where they differ on words only, the ADR owes a vocabulary-only correction at the next sweep.
5. **Index integrity:** every `<a id="open-X-NN">` anchor in the bible has exactly one row in chapter 8 and every row resolves to an anchor.

### Fixing

Missing sentences and boxes are written into the bible from the session's own held text; stale boxes are replaced by their answer and their index line removed. Part 3 is never rewritten.

**Skip if:** No discovery session has closed and no discovery-sourced ADR has changed since the last check.
...
1. **Stamp check.** Read the "Reflects decisions through" stamp at the top of `ARCHITECTURE_ANATOMY.md`. List every accepted ADR in `docs/architecture/decisions/` numbered above the stamp. For each, one glance: does it change what the overview or diagram shows (tiers, services, cores, ownership splits, contract boundaries, lifecycle states)?
   ...
   - **The same stamp check runs against `docs/architecture/DOMAIN_ENTITIES.md`** (added 2026-09-05, COR-E W8 / GC-24 — Audit V AC5-3 found it carrying an ADR-U046 stamp while citing ADR-U050 it had not absorbed): list accepted ADRs above *its* "Reflects decisions through" stamp; an entity-relevant one (a new entity, a state machine, an ownership move) → finding; none → move its stamp with a "reviewed, no entity impact" note.
2. **Index completeness.** Every `ADR-U*.md` file in `docs/architecture/decisions/` must have a row in `decisions/README.md`, and each row's status must match the ADR's own Status header. A file with no index row is a **finding in its own right** — the anatomy doc's own freshness note says "stamp vs the ADR index", so a stale index silently blinds any check (or reader) that trusts it. (Added 2026-08-10 at the AB-6 audit: ADR-U052 had been Accepted for ten days with no index row, and step 1 alone would never have flagged it.)
3. **Retired-vocabulary grep** over the living pair only (the Section 1 terminology list applies). Frozen snapshots (`ARCHITECTURE_ANATOMY_V1.md`, superseded `ECOSYSTEM_ANATOMY_V*`) are exempt from vocabulary checks — but each must carry its historical banner (MD) or SUPERSEDED watermark (SVG); a frozen snapshot without one is a finding.
4. **Pointer integrity.** These must resolve to the living doc and the current diagram version: the root `CLAUDE.md` document-map row, `PROCESS.md`'s companion-docs line, the architecture `README.md` tree + table, and any ADR "(current visual)"-style pointers. **Resolving is not enough — the version text must match** (added 2026-09-05, GC-24 — Audit V AC5-6: the README said v2.6 while the SVG was v2.7 for 25 days across a clean run): `grep -o 'v2\.[0-9]' docs/architecture/ECOSYSTEM_ANATOMY_V6.svg | head -1` must equal the version the architecture `README.md` tree line and table row give for that file, and the anatomy's "Diagram companion" line must agree. A pointer landing on a superseded version is a finding (fix in-place).
5. **Measurement-ledger append check** (same freshness family — a derived record lagging its events): every `*-gate-measurements.md` under `docs/planning/hub-v2/` dated after the newest row in `docs/planning/reference/PERF-MEASUREMENT-LEDGER.md` must have a ledger row (ADR-U052 clause 5 makes the append mandatory at every gate pass). A missing row is a finding — backfill it with a dated note naming the omission. (Added 2026-08-10 at the AB-6 audit: the RD-B pass ran its protocol perfectly and skipped only the append.)
...
**2026-09-05 — the pointer that resolved but lied (Audit V AC5-6, GC-24).** Section 11 step 4 checked that "current anatomy" pointers *resolve* to the living pair, not that the *version text* beside them matched the diagram: the architecture README said v2.6 from Audit IV's v2.7 refresh (2026-08-11) through the clean 2026-08-20 run. Same run, the same section had no eye on `DOMAIN_ENTITIES.md` at all, whose stamp (ADR-U046) sat below an ADR (U050) its own header claimed to follow. Fixed: step 1 stamps the entity file too; step 4 compares version strings. The general shape — *a pointer can be valid and stale at once* — joins gap 4's inverse-check family.
