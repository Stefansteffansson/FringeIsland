...
- **Claude Code owns all git for `discovery`.** The sweep — run at session start and again in the close ritual:
  ...
  4. If `docs/planning/sessions/openers/claude-ai-discovery-project-instructions.md` changed in the session, tell Stefan to re-paste its "Paste from here" block into the Claude.ai project — the project holds only a paste and nothing syncs it.
...
- **New files in `docs/fringeisland-thinking/` follow its naming convention** — `<register>--<area>--<topic>.md`, one level deep, no sub-directories (the folder's README explains the six registers). A discovery session is appended to Part 3 of `discovery--the-universe-in-the-making.md` (the one discovery file; statements numbered on from the last); a session's closing "Baked into the bible" list is carried into the Universe Bible (`bible--fringeisland-universe.md`) by Claude Code — a true sentence written, an Open box answered and its index line removed, a glossary entry corrected. A discovery session never writes into the bible itself.
