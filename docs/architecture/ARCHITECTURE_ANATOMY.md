...
**Reflects decisions through:** [ADR-U051](decisions/ADR-U051-actionable-notification-typed-response-framework.md) (2026-07-24) — reviewed at the A-NTF Cycle N-B close with **no anatomy impact**: U051 fixes the shape of a DS-5 contract family (typed, data-driven notification responses with permission-fanned shared actions) and the surface pattern that consumes it, but adds or moves no tier, service, core, ownership split, or contract boundary this overview draws. Substantively the anatomy still reflects [ADR-U049](decisions/ADR-U049-announcements-durable-home-routed-delivery.md) (2026-07-20). [ADR-U050](decisions/ADR-U050-account-lifecycle-state-machine.md) is **Proposed** (rides the C-F schema gate) and is deliberately not absorbed here until it is Accepted.
...
- **PC-2 Identity** — authentication, sessions, profiles, the `user_id` contract, the **Mist lifecycle** ([ADR-U031](decisions/ADR-U031-mist-identity-lifecycle.md) — the anonymous entrant is the **Mist**; "Shadow" names the place-3 menace).
