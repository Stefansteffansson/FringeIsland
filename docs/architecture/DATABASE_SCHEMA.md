...
CREATE INDEX idx_memberships_active ON group_memberships(group_id, status) 
  WHERE status = 'active';
```
...
CREATE INDEX idx_enrollments_active ON journey_enrollments(journey_id, status) 
  WHERE status = 'active';
```
...
-- Seed data (initial role templates)
...
2. Create groups and group_memberships tables
3. Create journeys and journey_enrollments tables
4. Create authorization tables (permissions through user_group_roles)

### Phase 2: Seed Data
1. Insert initial permissions
2. Insert initial role templates
3. Create role_template_permissions mappings
4. Insert group templates
5. Create group_template_roles mappings
