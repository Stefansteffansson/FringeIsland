...
**Status:** twenty-eight known documentation and design gaps across the FringeIsland development system as of 2026-05-01.
...
| G-30 | Agent routing | Tier CLAUDE.md files contain miscategorised entity-specific content | Medium | Walk each tier `CLAUDE.md` against the four-row content policy in root `CLAUDE.md`; migrate Hub-specific rules out of `products/CLAUDE.md` and Core/Domain-specific guidance out of `platform/CLAUDE.md` into entity- or sub-tier-level files |
| G-31 | Decomposition (L3 methodology) | Stress-test pass not yet named in `ecosystem-decomposition` skill | Medium | Add a methodology-step section to `ecosystem-decomposition` covering the three-step pattern (cold derivation → code-informed stress-test pass → adjudication), the three output classes, and the routing connection to G-29 |
...
The 2026-04-30 stress-test pattern bridge (`docs/planning/sessions/2026-04-30_01_-_CODE-INFORMED-STRESS-TEST-PATTERN.md`) names a three-step L3 authoring pattern (cold derivation → code-informed stress-test pass → adjudication) whose stress-test step produces a structured delta in three classes. Two of the three classes (confirms, entity-internal delta) have natural homes inside the candidate inventory itself. The third class — **cross-entity findings** (empirical or architectural artifacts that don't belong to the candidate entity at all but to another entity entirely) — has **no canonical home**. Without a routing mechanism, these findings either get silently dropped, get captured ad-hoc inside the candidate entity's spec where they don't belong, or live in session-bridge memory until they're forgotten. The Hub L3 derivation (Block B.2) is the first instance producing such findings; cascade-plan Session 4 (tier-CLAUDE content audit, which applies the stress-test pattern to a different decomposition surface) is the natural second instance. Without lateral routing, downstream entity L3 sessions will not inherit the pickup-context that prior entities' stress-test passes generated for them, and the structural-completeness probe value of the stress-test pattern is lost.

*Proposed fix:* design a routing mechanism. Don't pre-decide the shape — candidate options noted in the originating B.2 bridge include (a) per-entity expected-dependency appendix in every L3 spec, template-wide; (b) separate registry file (e.g., `docs/architecture/expected-dependencies.md`); (c) follow-up notes appended to each entity's pickup list. Each has different ergonomics for the entity author writing the finding, the entity author later reading the finding, the cycle-boundary review that ensures findings don't accumulate unread, and the `doc-health-check` skill that would verify the mechanism's discipline holds. The resolution session designs the mechanism and updates `ecosystem-decomposition` skill to make the routing step part of the L3 stress-test pattern's adjudication phase. **Connection to G-31:** G-31 covers naming the stress-test pattern as a methodology step in `ecosystem-decomposition`; G-29 covers routing the cross-entity findings the pattern produces. The two gaps' resolutions are complementary but separable — G-29 can resolve before G-31 (the routing exists; per-session discipline names the producer), or G-31 can resolve before G-29 (the methodology step is named; the routing remains a per-session caveat in sources-status until G-29 resolves).

**G-31 — Stress-test pass not yet named in `ecosystem-decomposition` skill (decomposition, L3 methodology).**
The 2026-04-30 stress-test pattern bridge (`docs/planning/sessions/2026-04-30_01_-_CODE-INFORMED-STRESS-TEST-PATTERN.md`) names a code-informed stress-test pass methodology step — architecture-derived inventory first, code-informed comparison second, three-class delta with cross-entity findings routed laterally. Block B.2 (Hub L3, closed 2026-04-30) is the first instance, producing seven methodology observations (Observations A–G in the closing bridge `2026-04-30_02_-_BLOCK-B2-HUB-L3-CLOSE.md`) of which Observation A — that cold derivation cannot determine which L2-committed capabilities are implemented vs planned — structurally validates the pattern as necessary rather than merely efficient. Without naming the pattern in the skill, future entity L3 sessions (Gimbal, Studios, Domain Services, Platform Core areas) re-derive these observations from session-bridge memory or skip the stress-test step entirely and produce under-classified inventories. The pattern's value is reproducibility; without a named home in `ecosystem-decomposition`, it is not reproducible.

*Proposed fix:* add a sub-section within Level 3 of `ecosystem-decomposition` skill — likely at the end of L3 — that names the three steps, the three output classes (confirms / entity-internal delta / cross-entity findings), the cold-derivation discipline (no reference to existing artifacts during step 1), and the routing connection to G-29 for cross-entity findings. The cascade-plan bridge (`docs/planning/sessions/2026-04-27_01_-_AGENT-CONTEXT-CASCADE-PLAN.md`) sequenced this fix as deferred until two-instance evidence; the closing bridge of B.2 produced Observations A–G as load-bearing evidence beyond what one data point would have given, and the fix is being landed in cascade-plan Session 1 against that material rather than waiting for Session 4. Worth noting that Session 4 then becomes a *validation* of the named pattern rather than a generator of it; Session 4 refinements update the skill text in place.
...
- G-31 Stress-test pass not yet named in `ecosystem-decomposition` skill
