...
**G-33 — Universe-discovery Session 01 Open-threads list is stale / uncurated (documentation hygiene).**
The Open-threads section of `docs/ecosystem/thinking/universe-discovery/2026-05-18_universe-discovery-session-01.md` grew append-only across the session's resumes (2026-05-18 / 05-20 / 05-27 / 05-29) but resolved threads were almost never pruned or marked. Only the Live/Grow/Matter balance thread (under Statement 28) carries an inline "RESOLVED". Statements 26, 27, 30 and the 2026-05-29 resume (Statements 31-36) each answered multiple threads that still sit in the list looking open; the 2026-05-29 resolutions are captured only as a single summary line at the top of the section, not against the individual bullets. The live to-do therefore misrepresents what is actually still open. Surfaced 2026-05-29, immediately after the Statements 31-36 commit (3548b98).

*Proposed fix:* a dedicated curation pass. Read every Open-thread bullet against all 36 statements and classify each as still-open / fully-answered / partially-answered. Do NOT delete answered threads (that erases the audit trail of what was open and when it closed); instead either mark each resolved one inline (`RESOLVED -> Statement N`, as Statement 28 was done) or split the section into "Still open" and "Resolved (with resolving statement)" subsections. Drafting/review work; deliverable is a proposed open/resolved split for Stefan to approve, then a single corrective commit separate from any authoring. Candidate enforcement: a `doc-health-check` extension that flags a session's Open-threads list when later statements plausibly resolve a thread (heuristic surfacing; manual adjudication remains the mechanism).
