...
**G-03 — Vertical specs are scaffolds (cross-cut).** **Highest-priority gap in the whole document.** *Status update (2026-07-31, Cycle ADM-A / TASK-OBS-01):* the blanket claim below is stale — all five specs were substantively populated by the 2026-06-12 verticals derivation (tier obligations, checklists, failure modes, open questions all real). **The V4 Observability slice is now also reconciled to realized tooling**: ADR-U052 selected the Ferd sink (PC-1 `telemetry_events`, FEAT-PC018), the durable-sink obligation joined §6, and three §5 open questions are answered/advanced in place. Remaining under this gap: a per-vertical staleness review of the other four §3 tooling sections against today's substrate (the same reconciliation V4 just got), and the tier-CLAUDE sync check the fix below names — narrowed, not closed.
