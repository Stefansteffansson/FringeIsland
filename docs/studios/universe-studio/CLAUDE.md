...
- **The parent writes to no Domain Service.** Each child writes to exactly one (World Studio → DS-1 World Model; Arc Studio → DS-2 Narrative Engine; Journey Studio → DS-3 Experience Engine), per the studios-tier file.
