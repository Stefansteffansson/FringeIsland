...
**Amendment 2026-09-03 ([TASK-SEC-02](../../../planning/backlog/tasks/TASK-SEC-02-table-grant-narrowing-and-truncate-sweep.md), migration `20260902210000`, applied on the named approval).** The narrowing shape below (Open Q4: INSERT/TRUNCATE revoked, UPDATE kept as a **column grant** on the settable columns) is superseded: the client roles now hold **no** DML on `public.groups` at all — the column-scoped UPDATE is revoked with the rest of the table set. The Hub never used the direct path (`PATCH /api/groups/[id]` goes through `update_group_settings()`, the one door), and its only pin, the regression-guard cell *"still allows the permitted Steward to update a settable column directly"* in `group-crud-contracts.test.ts`, is **inverted** with a label. The `groups_update` policy stays as written; a policy without a grant permits nothing. **Classify by sense (doc-health):** a description of the G-A narrowing as what shipped in July is correct history; a claim that a Steward can write a settable column *directly today* is drift.
...
## Amendment (2026-07-04 — creator participation binding; Stefan's decision)

Found during G-D manual testing: the creator was bound to the **management role only** — a member of their own group (active membership row) but holding no participation bundle, so the one thing a creator could not do in their own group was `enroll_self_in_journey`. Decision: **creating a group means stewarding it AND taking part in it.** Migration `20260704204343` replaces `create_engagement_group()` in place: after the mandatory permission-derived management binding (`assign_roles` marker), the creator is additionally bound to the **participation role** — permission-derived via the `enroll_self_in_journey` marker (the Member/Participant template today; the role-name/rename question stays with the roles core), **soft-skipped** when the instantiated role set carries no participation role (facilitation-only templates stay legitimate), removable afterwards like any binding. Red-first: the crud suite's creator-binding assert re-specified to two bindings + the `enroll_self_in_journey` round-trip (red at 1 binding → green post-migration). No signature change, no new table, no policy changes.
