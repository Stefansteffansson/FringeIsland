...
- **`admin_get_content_report_detail(p_report_id UUID) → jsonb`** — the row keys plus the live-resolved escalation keys: `author_user_id` + `author_display_name` (re-resolved from `forum_posts`/`messages` at read time — **nullable**: deleted content resolves to NULL while the snapshot stands as the record), `live_target_exists` (drift honesty), and `resolved_by_display_name` on resolved rows. Refusals: `42501`, `P0002` (existence-hiding).
...
- Given the reported content has since been deleted, then the author keys are NULL, `live_target_exists` is false, and `content_snapshot` still carries what the content said when reported (the C-D drift rule doing its job).
...
- Given the reporter was erased before resolution, then the resolve succeeds and no notification exists (the CASCADE took the row).
