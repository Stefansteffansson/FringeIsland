...
3. **The audit log's client write door is still open.** `audit_log_insert_admin` (`20260223164813`) lets any admin-authenticated PostgREST caller INSERT arbitrary field-content rows — pattern (c) of the governance §L4 three-pattern note (7 legacy UI sites, all in `hub-legacy/` and none in v2 — as written they were *frozen*; the tree was **deleted** at the Phase-4 cutover 2026-08-11, so those call sites no longer exist at all). Under the v2 law (contracts are the only door) and the ADR-U038 direct-caller question, a forgeable audit log fails its own integrity purpose.
