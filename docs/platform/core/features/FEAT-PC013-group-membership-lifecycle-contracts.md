...
- **`get_group_detail` amendment (additive)** — member rows gain `membership_status`; **paused rows are included only for viewers holding any of `pause_members` / `activate_members` / `remove_members`** (Open Q3 — membership state is FIM data, org-spec Privacy note; every other viewer sees the active-only list unchanged). Management keys also imply member-list visibility itself — you cannot manage what you cannot see (surfaced at build by the minimal-permission pauser persona). `member_count` stays active-only. (PC011 already added `member_group_id` + `roles[]`; this rides the same payload.)
...
- Given a non-member, an invisible private group, a ghost id, or a non-engagement group, then `P0002` indistinguishably (the legacy `'Group not found'` / `'not an active member'` string leak closes; visibility precedes every other check — non-engagement ids resolve exactly like absent ones, so nothing is enumerable); given a non-active engagement group the caller is a member of, then `P0001`; given a Mist, then `42501`. *(AC amended 2026-07-04 at build start: the original "personal/system group → P0001" ordering would have leaked non-engagement existence ahead of the visibility wall.)*
