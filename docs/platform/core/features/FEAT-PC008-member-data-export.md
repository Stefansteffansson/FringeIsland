...
### Amendment (2026-07-19, COR-A W8 — the forward seams realized)

The v1 forward seams are closed: `get_own_data_export()` now composes the **`journal`** (`get_own_journal_export()`, FEAT-PD001) and **`journeys`** (`get_own_step_instances_export()`, FEAT-PD007) sections platform-side, by calling the owning contracts — sanctioned by **ADR-U047 Amendment 2** (vertical-obligation composition: the Privacy/GDPR export may compose Domain services' published read contracts; this supersedes this spec's "PC-4 does not read Domain tables → sections contributed as forward seams" framing for the export case). Export **completeness** is now this contract's responsibility (anatomy-audit AC-4); the Hub route is a thin proxy again. **`schema_version` stays 1 deliberately** — a conscious departure from STORY-5's "new section ⇒ version bump" rule: the *delivered* document has carried exactly these keys since the Hub route composed them route-side (FEAT-H011 / FEAT-H024), so the composer moved but the wire shape did not change; the bump rule applies to sections genuinely new to the wire. Known inherited asymmetry (open spec question for the owning areas): the walks section resolves its actor via the `is_active`-gated helper, so a suspended member's composite export raises 42501 — at odds with this spec's suspended-members-retain-access posture; documented in the migration header. **CLOSED at C-E (2026-07-21, CB-6 — right of access adopted at the C-E board):** `get_own_step_instances_export()` re-issued with this spec's own ungated resolution (`auth.uid()` → `public.users` direct; `get_current_personal_group_id()` untouched), and the new `communication` section (`get_own_messages_export()`, [FEAT-PD012](../../domain/features/FEAT-PD012-lifecycle-dispositions-and-export-contracts.md)) is born ungated — a suspended member's full composite now returns, proven red→green (`communication-export.test.ts` STORY-5). Migration `20260721100000`, PR #230. Migration `20260719201718`, PR #191; tests: `export-composite.test.ts` (red pre-apply → green) + STORY-5's omitted-sections assertion updated.
