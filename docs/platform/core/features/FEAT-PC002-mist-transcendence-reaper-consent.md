...
### Amendment 2026-09-04 — reaper_runs retention (migration `20260904100000`)

The run log had no retention rule: one row per 15-minute `mist-reaper` run since 2026-06-26, 7,400 rows (all `success`, 239 Mists erased, zero errors) by the 2026-09-04 database review, growing about a hundred a day forever, while `telemetry_events` had its nightly prune. Stefan: "Fix this please" — the named approval. Added: `prune_reaper_runs()` (SECURITY DEFINER, `search_path ''`, deletes runs older than **30 days**, returns the count; EXECUTE for `service_role` only — an operations job, never a member door), `reaper_runs_ran_at_idx`, and cron job `reaper-runs-prune` nightly at 03:45 (after `telemetry-prune`). One prune at apply, self-verified. Thirty days keeps about a month of reaper health for the admin surfaces this spec reserved ("admin surfaces consume later"). Red-first: `hub/tests/integration/auth/reaper-runs-retention.test.ts` — the member's 42501, the prune's keep/delete boundary and count, the job's presence and schedule; the two sibling jobs pinned as labelled green. Registered under PC-2 in the ownership manifest.
