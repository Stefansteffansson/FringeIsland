...
- **Notifications:** **no new kinds — the CB-1 board ruling, recorded as a dated V3 deferral (2026-08-01):** affected-member sanction communication is deferred to the ADM-D DS-5 kind family (or Eid), activation named there. The slice composes only **existing** kinds: `notify_role_assigned` on grant; `stewardship_transferred`/`stewardship_required`/`group_closed` inside the walks — exactly as the self walk fires them.
