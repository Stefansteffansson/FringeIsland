...
4. **[SUPERSEDED 2026-08-05 — see the WA-6 Amendment at the end of this spec: template-less instantiation now carries the SYSTEM templates only, clones pull-only, migration `20260805150000`. The statement below is the substrate as found at spec time.]** **The template-less instantiation path copies EVERY role template** (`create_engagement_group` selects via `group_template_roles` for a chosen group template *or every role template when none chosen*). Consequence: any new `role_templates` row — including a clone — rides every future template-less instantiation, and appears in `get_role_templates()` (the member-facing group-creation options). The editor must say so; the contracts must pin it.
...
- **WA-3 — `admin_hard_delete_user` re-issue:** the consent-erasure leg — **anonymise the subject link, don't purge** (build-time L4 correction to the walk's working wording, 2026-08-04): NULL `consent_records.subject_user_id`/`subject_group_id` for the target under `PERFORM set_config('app.consent_erasure_in_progress','true',true)` **before** the personal-group cascade, copying `erase_fim_account`'s leg verbatim (`20260627120000:83-91` — ADR-U034 §5's anonymise-then-retain: the consent events survive as GDPR proof; a purge here would destroy that proof on the composed `erase_fim_account → admin_hard_delete_user` path). Idempotent under composition (the outer anonymise leaves zero matching rows for the inner). The audit-before-delete row, DS-5 reattribution, DS-3 cleanup, and cascade order stay verbatim. The gate suite gains **the consented-member cell the family never had** (cross-ref: TASK-E2E-02 — the same FK silently leaked 1,289 E2E fixtures).
