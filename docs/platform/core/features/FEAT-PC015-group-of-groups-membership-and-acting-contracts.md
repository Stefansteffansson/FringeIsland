...
### Post-6-done fixes (2026-07-06, live testing)

- **Revealed visibility on `get_group_detail`** (migration `20260706170000`, replaced in place): two viewer cases open the group's FACE — the caller's own `invited` membership (active groups only; look before you answer) and wielding an active member-group (ADR-U041 §2a substitution; any lifecycle state). Member-list rules, paused-darkness (PC013 Open Q3), and the stranger P0002 posture unchanged (no-over-widening guard-tested). **Rider:** the verification block surfaced `get_group_detail`'s PC010-era anon EXECUTE (one of the twelve PC014-documented sweep items) — revoked in place; the parked pre-partition grant sweep shrinks by one. Surface half in FEAT-H018's Post-6-done fixes.
- **`get_acting_contexts` became context-scoped** (migration `20260706150000`, DROP + CREATE — the return shape gained `is_member_of_context` and the signature gained `p_context_group_id default null`; a CREATE OR REPLACE would have left the zero-arg overload alive). With a context named, each wieldable group is flagged for ACTIVE membership in it, so the Surface offers only hats with standing (self is never flagged — a group is never a member of itself). Unscoped behavior unchanged; grants re-stated post-drop, anon verified zero. The Surface half of the finding lives in FEAT-H018's Post-6-done fixes.
