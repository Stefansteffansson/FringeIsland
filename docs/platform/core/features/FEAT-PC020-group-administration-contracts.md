...
| detail members summary (counts only in v1) | detail counts block |

Every key has a consumer; the reassign picker's candidate list comes from the **existing** group-membership read (walked: `get_group_memberships_of` serves it — a second payload the surface consumes, per the J-D rider rule), not from a new contract.
