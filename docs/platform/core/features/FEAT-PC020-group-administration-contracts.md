...
## Amendment (2026-08-03, FEAT-PC023 / Cycle HYG-A)

The availability-ceremony semantics this spec shipped are extended by [FEAT-PC023](./FEAT-PC023-group-suspension-enforcement-contracts.md) under the two-mode hold model (Active / Resting / Suspended): `admin_suspend_group` now admits the `resting` origin (`active|resting → suspended`; refusal copy updated to "cannot suspend a group that is not active or resting"; the audit row's `previous_status` metadata carries the origin as before), and two thin audited wrappers join the family — `admin_rest_group` / `admin_wake_group` (audit actions `group.rest` / `group.wake`), composing the PC023 member-plane contracts `rest_group()` / `wake_group()` per the ADM-18 idiom. `admin_reactivate_group` is unchanged (`suspended → active` stays the only exit from the hard state). Migration: `20260803190000_hyg_a_pc023_group_availability_enforcement.sql`.
