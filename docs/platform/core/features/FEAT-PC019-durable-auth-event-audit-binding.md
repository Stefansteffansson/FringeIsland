...
- **Append-only untouched:** no UPDATE/DELETE policies exist on `admin_audit_log` and none are added; the existing `is_platform_admin()`-gated SELECT/INSERT policies are untouched (the legacy pattern-(c) door was `hub-legacy/`'s; v2 never uses it, and the tree itself was deleted at the Phase-4 cutover 2026-08-11 — narrowing the door is an ADM-D question, noted, not built here).
