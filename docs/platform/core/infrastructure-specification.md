...
consumers: [platform/domain/world-model, platform/domain/narrative-engine, platform/domain/experience-engine, platform/domain/content, platform/domain/communication, platform/domain/discovery, platform/domain/intelligence, platform/extensions, products/hub, products/gimbal, products/game, studios/journey, studios/universe, studios/arc, design-system]
status: proposed
...
- **SQL helpers and primitives:**
  ...
  - Trigger conventions — the patterns and naming for validation triggers and audit-substrate triggers.
...
- **Connection-role conventions** — what `anon`, `authenticated`, and `service_role` are allowed to do; how RLS is evaluated under each; which operational paths legitimately use `service_role` and which do not.
