...
**Authorship note.** L2 owns §L2 below (identity, boundaries, technical shape). L3 owns §L3 (capability inventory). L4 owns the feature-inventory summary (§L4 — first populated 2026-07-31 with PC-1's first feature). No level modifies a section owned by another.
...
- **SQL helpers and primitives:**
  ...
  - Scheduled-job substrate (pg_cron) — the canonical scheduler convention for recurring database jobs (added 2026-06-10, DS-1 descent Phase 0). PC-1 owns the extension enablement, job naming conventions, and the discipline that sweep jobs honour consumer-declared guards; job definitions themselves are owned by the consuming area (e.g., PC-2's Mist TTL sweep per ADR-U004/U031, which must honour the explicit-erase path and the mid-migration guard).
...
- **Migration discipline:** migrations run in timestamp order; never rewrite an applied migration; the apply-then-mark-applied two-step (`apply-migration.js` → `supabase-cli.sh migration repair --status applied`) is the canonical workflow per `platform/CLAUDE.md`.
...
| **Telemetry event substrate** *(added 2026-07-31, A-ADM area open — see reconciliation note below)* | PC-1 | Schema management *(table rides the migration discipline)*; RLS substrate *(deny-all posture)*; Privilege-escalation discipline *(SECURITY DEFINER recorder)*; Scheduled-job substrate *(90-day retention prune)* | — | Observability *(the durable V4 sink — ends the emit-without-store state, AC3-O7)*; Privacy *(actor-linked events are personal data: erasure cascades with the personal group, retention-bounded, content-free per ADR-U052 §4)*; Administration *(the statistics read aggregates from it)* |
| **Statistics aggregation** *(added 2026-07-31, A-ADM area open — the load-bearing ADM-1 commitment Hub §L3 attributes to PC-1; see reconciliation note below)* | PC-1 | Telemetry event substrate *(activity trends)*; RLS substrate; Privilege-escalation discipline *(admin-gated SECURITY DEFINER read)* | — | Administration *(the admin dashboard's read — platform-admin-gated, typed refusal)*; Observability *(computed on read — no shadow aggregate tables to drift, ADR-U052 §3)*; Privacy *(aggregates are counts; no per-member disclosure)* |
...
**Reconciliation note — Telemetry event substrate + Statistics aggregation (added 2026-07-31; the G-35-style forward-addition pattern).** These two rows were **not** in this inventory at the Phase-2 PC-1 derivation (2026-05-04) because their decision layer did not exist — the sink/retention/consent posture is decided by [ADR-U052](../../architecture/decisions/ADR-U052-telemetry-sink-and-analytics-posture.md) (2026-07-31, the A-ADM area-open board AB-1), which post-dates the derivation. They are **forward additions driven by that ADR, not corrections of a derivation miss.** Authority for PC-1 ownership: `docs/products/hub/SPECIFICATION.md` §L3's ADM-1 row (*"PC-1 (statistics aggregation — load-bearing infrastructure commitment)"* — previously attributed-but-unenumerated, the same shape PC-4's G-35 notes record), TASK-OBS-01 (discharged by ADR-U052 + FEAT-PC018; file swept at the A-ADM close — disposition in the [tasks README](../../planning/backlog/tasks/README.md) sweep log), and the [A-ADM plan](../../planning/hub-v2/phase-3-platform-ops-completion-plan.md). Both are realized by [FEAT-PC018](./features/FEAT-PC018-telemetry-event-store-and-statistics.md) ↔ Hub [FEAT-H034](../../products/hub/features/FEAT-H034-admin-dashboard-and-durable-audit-wiring.md). This is a deliberate backward-edit to a ratified Phase-2 inventory, scoped to the two grounded capabilities, and rides the held ADM-A decomposition PR for review at the parent scope.
...
*L4 authorship. Reconciliation output against §L3. First populated 2026-07-31 (Cycle ADM-A) — until then this section was deliberately absent (the Phase-2 L1→L3 scope decision, 2026-05-03) so an empty section could not masquerade as "no features owned by PC-1." Maintenance discipline: the `feature-development` skill updates this section at maturity transitions; `doc-health-check` Section 8 verifies it reflects `features/`.*

| Capability (from §L3) | Feature spec | Maturity | Notes |
|---|---|---|---|
| Telemetry event substrate + Statistics aggregation | [FEAT-PC018](./features/FEAT-PC018-telemetry-event-store-and-statistics.md) | 6-done | PC-1's **first feature spec**: the deny-all `telemetry_events` store, the never-raises `record_telemetry_event()` recorder, the pg_cron 90-day prune, and the admin-gated computed-on-read `get_platform_statistics()` — the [ADR-U052](../../architecture/decisions/ADR-U052-telemetry-sink-and-analytics-posture.md) realization. Platform half of Hub [FEAT-H034](../../products/hub/features/FEAT-H034-admin-dashboard-and-durable-audit-wiring.md); Cycle ADM-A ([plan](../../planning/hub-v2/phase-3-platform-ops-completion-plan.md)). |

The remaining §L3 capabilities (schema management, RLS substrate, privilege-escalation discipline, trigger-based validation, object storage, feature-flag substrate, scheduled jobs, connection roles, extension management) are substrate exercised through every other area's features rather than specced as PC-1 features; they gain FEAT-PC-* specs only if a consumer demands a new contract (the feature-flag substrate is explicitly deferred — A-ADM board AB-8 / ADM-15).
