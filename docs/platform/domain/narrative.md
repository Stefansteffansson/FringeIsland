...
- **Calendar reads** — current season and episode on the universal calendar; the schedule ahead; the unfolding so far (what has aired). Published narrative structure is shared-world state, open to Shadows and FIMs (stories stand alone as entertainment; the near side is anchor-free).
...
- **Platform Core:**
  ...
  - **PC-2 Identity** — identity-status gate (Shadow/FIM) for read-posture only: published narrative structure is anon-readable shared-world state; DS-2 holds **no per-Shadow or per-FIM personal state** at this derivation (structure plus Dreamineer authorship attribution only), so ADR-U027 ephemerality obligations attach only if a future capability adds per-Shadow state.
...
- Every DS-2 table has RLS from day one. **Published** narrative structure (current/past seasons, episodes, arcs, beats, loop declarations, character layers) is readable by `anon` — stories stand alone as entertainment and Shadows experience the near-side world. **Draft/unpublished** authoring state is Teller/studio-scoped, enforced at the row grain via lifecycle state.
...
- **Q8 — The beat-vs-content-block boundary (DS-4).** A beat is structure (position, sequence, arc membership, loop role); a block is content (the media/text the beat presents). Cold lean: beats reference blocks opaquely by ID and DS-4 owns everything renderable. Confirm at the DS-4 descent.
...
| PC-2 Identity | Identity-status gate (Shadow/FIM); four-hop actor chain | read-posture on published structure (anon-readable); every actor-touching write |
