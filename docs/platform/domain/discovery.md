...
8. **Shadow ephemerality and anon parity.** Shadows perceive the real published shared world (ADR-U027: the privacy protection is ephemerality, not refusing to serve) — anon-readable discovery mirrors the sources' anon-read posture; FIM-only reaches (the village, deep place 3 — S45) never enter Shadow-visible surfaces because their sources never publish them anon-readably. Shadow-generated discovery traces inherit TTL-erasure.
...
- **Q5 — Search history and saved searches.** Are traveller search traces retained at all? If yes: personal data, TTL posture, Shadow-trace ephemerality (ADR-U027), Privacy-vertical consent surface. Cold lean: not retained at Ferd.
...
| Shadow/anon read posture & personal-trace ephemerality | Index & visibility lifecycle | — | PC-2 (identity status; ADR-U027 TTL-erasure) | Privacy (anon parity with sources; traces TTL-erased); Observability (posture evaluations on denial) |
...
7. **Shadow/anon read posture & personal-trace ephemerality** — cross-cutting posture, binds from the first read surface.
