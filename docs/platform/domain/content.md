...
|--------|-----------|--------------|
| Asset | A registered unit of media: 3D model, image, audio, captured scan — **kinds data-driven** (Ferd non-closure; new media kinds addable without schema change). Carries authoring provenance, draft/published lifecycle, and a storage-object pointer. The NPC body layer (the bible's beings chapter), placed world objects (DS-1 references), and block-embedded media all resolve to asset rows. | DS-4 tables (substrate; schema is L4) |
...
| Capture record | The intake record of a raw capture from the sensors foot (Gimbal): provenance (who, when, what equipment), rights posture (§8 Q3), and the capture's personal-by-default privacy state. **A Mist's captures inherit ADR-U031 ephemerality.** | DS-4 tables (Mist captures inherit PC-2 ephemerality) |
...
- **Platform Core:**
  - **PC-1 Infrastructure** — RLS substrate; SECURITY DEFINER discipline; migration discipline; **object-storage conventions** (the substrate row DS-1's placed-asset references already cite; ADR-U003 Supabase Storage); **scheduled-job substrate (pg_cron)** for Mist-capture TTL sweep participation; feature-flag substrate (staged rollout of capture/refine and authoring surfaces).
  - **PC-2 Identity** — identity-status gate (Mist/FIM) and the four-hop actor chain on every actor-touching operation; **Mist ephemerality rules** (a Mist's captures and authored content inherit the TTL-erasure obligations of ADR-U031); the **transcendence lifecycle event with atomic migration** (a transcending Mist's captures migrate with continuity — composed invariant with PC-2's migration, cascade-spec'd per ADR-U016).
...
- Mist-generated DS-4 state (a Mist's captures and anything authored from them) carries the PC-2 TTL-erasure path; pg_cron is the sweep mechanism; the transcendence migration is atomic with a mid-migration guard (ADR-U031), cascade-spec'd per ADR-U016 jointly with PC-2 before implementation — blob erasure in the storage substrate is part of the sweep's obligation, not just row deletion.
...
4. **Mist-capture ephemerality.** Mist-generated content state inherits TTL-erasure and the atomic transcendence migration (ADR-U031) — including the storage blobs, not only the rows. A last-moment transcender's captures are never erased mid-migration.
...
| Capture intake | Capture → refine pipeline | Asset registry & kinds; Storage-object lifecycle | PC-2 (identity-status gate; ADR-U031 ephemerality on Mist captures); PC-1 | Privacy (captures personal-by-default, invariant 6; real-world rights posture §8 Q3); Observability (intake events) |
...
| Mist-capture ephemerality | Lifecycle | Capture intake | PC-2 (ADR-U031 TTL-erasure + atomic transcendence migration); PC-1 (pg_cron sweep; blob erasure in the storage substrate) | Privacy (ephemerality obligations incl. blobs, invariant 4); Administration (atomicity cascade-spec'd jointly with PC-2 per ADR-U016) |
...
11. **Mist-capture ephemerality** — over intake; needs PC-2's transcendence-event contract (cascade-spec'd before build).
...
| PC-1 Infrastructure | Object-storage conventions (ADR-U003 Supabase Storage) | Storage-object lifecycle; Capture intake; Mist-capture ephemerality (blob erasure) |
| PC-1 Infrastructure | Scheduled-job substrate (pg_cron) | Mist-capture ephemerality (TTL sweep participation) |
...
| PC-2 Identity | Mist ephemerality (TTL + explicit-erase per ADR-U031) | Capture intake; Mist-capture ephemerality |
| PC-2 Identity | Transcendence lifecycle event (atomic migration per ADR-U031) | Mist-capture ephemerality (composed atomicity invariant per §6) |
...
| Vertical: Privacy | Capture/personal-data obligations; Mist-data obligations; publication-consent posture (§8 Q3) | Capture intake; Refinement pipeline state; Content authoring write-path; Mist-capture ephemerality |
| Vertical: Observability | Event shape (request ID, actor, outcome) | every event-emitting capability |
| Vertical: Administration | Cascade-spec format per ADR-U016 | Content retirement cascades; Mist-capture ephemerality (transcendence atomicity) |
...
- **ADR-U025 + the 2026-06-05 product locks (load-bearing authority):** solid — the capture→refine pipeline ("scan a real object on the phone, refine it on the canvas — two ends of one workflow"), the equipment-profile model, and World Studio's capture-foot/deep-edit split consumed directly. **DS-4 is the first Domain Service deriving primarily from the product locks rather than a dedicated canonical core** — the canon-sub-page-gap remark per the DS-3 precedent: no `content.md`-shaped core exists; the bible's story chapter's planned `journeys.md` sub-page (content families) is unwritten. Proceeded with remark; §8 Q1 carries the vocabulary half.
...
- **L2-line altitude:** the domain README line "Media, assets, narrative blocks" predates ADR-U025 — it omits the capture→refine pipeline, renditions/variant delivery, pipeline state, Mist-capture ephemerality, the authoring write-path, and rendering contracts. Revision proposed at Step 3 of this run.
