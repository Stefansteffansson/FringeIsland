...
- ~~No enrolment pause (`paused` is a CHECK value with no write path — stays that way this cycle; recorded, not built).~~ **Built 2026-09-03 (STORY-8, TASK-JRN-PAUSE-01):** own-enrolment pause/resume only. Still out: no group-enrolment pause (a group's walk pauses by the group's lifecycle, not one member's act), no pause reason, no notification, no pause-aware ordering in any read.
...
### STORY-8: Pause and resume my own walk (JRN-19 platform half — amendment 2026-09-03, TASK-JRN-PAUSE-01)
As the platform, I want a traveller's own enrolment to pause and resume through contracts, so a rest is a recorded state the freeze cascade still reaches — never a fourth terminal, never a client fiction.

**Acceptance criteria:**
- Given a traveller (a FIM, or the Mist on their onboarding walk) with an `active` own enrolment, when they call `pause_journey_enrollment(enrollment_id)`, then the row is `paused` (`status_changed_at` moves; `progress_data`, step instances and engagements are untouched) and the reply carries `{enrollment_id, journey_id, status: 'paused'}`.
- Given a `paused` own enrolment, when they call `resume_journey_enrollment(enrollment_id)`, then it is `active` again and `get_player_state` resumes at exactly the position it held before the pause.
- Given a paused enrolment, when any step contract is called on it (`enter_journey_step`, `complete_journey_step`, `save_step_response`), then `P0001` — the pause holds until resumed.
- Given a `completed`, `frozen`, or `withdrawn` enrolment, when either contract is called, then `P0001` naming the state; given an `active` one on resume, or a `paused` one on pause, then `P0001` naming that.
- Given an enrolment that is not the caller's own (a via-group row they can see, even holding `unenroll_from_journey`), when they call either contract, then `42501` — a group's walk is the group's; given one they cannot see (or none), then `P0002`.
- Given a direct PostgREST caller, when they UPDATE `journey_enrollments.status` to or from `paused`, then the write is refused at the grant (`42501`, the TASK-SEC-02 posture — before RLS's silent zero rows).
- Given a paused enrolment in a group's non-public journey, when the ADR-U016 cascade fires (the traveller leaves or is removed; the group closes or is deleted), then the row is `frozen` with the cascade's `frozen_reason` — the frozen shape wins; pause never shelters a walk from the cascade.
- Given `get_my_enrollments`, `get_journey_detail`'s viewer block, and `get_player_state`, when the enrolment is paused, then each carries `status: 'paused'` as-is — no read reshapes it.
...
- **Notifications:** group enrolment writes durable notification rows to active members (self-enrolment is self-originated — none); push/preferences ride the Notifications area (ADR-U039). **STORY-8 (2026-09-03):** pause/resume emit nothing — the enrollee's own act, no one else to tell.
- **Administration:** enrolment participates in the existing ADR-U016 cascades (freeze on leave/close/delete — PC013/PC014, re-verified at J-D); provenance via `enrolled_by_group_id`; withdraw semantics recorded at the gate (Open Q1). **STORY-8 (2026-09-03):** the two DS-3 lifecycle-fact handlers (`ds3_lifecycle_member_departed`, `ds3_lifecycle_group_closed`) are widened from `status = 'active'` to `status in ('active', 'paused')` so a paused walk freezes exactly as an active one — the frozen shape wins; ADR-U047 rule 7's "active-only" description of the sprint2 shape is superseded for paused rows (recorded here and in the migration header, not silently fixed).
...
**Amendment 2026-09-03 (TASK-JRN-PAUSE-01, STORY-8 — migration `20260903100000`, applied 2026-09-03 on the named approval).** `pause_journey_enrollment` / `resume_journey_enrollment` on the withdraw shape (actor + account gate, the visibility mirror, then own-row, then state); no FIM-only gate — the actor gate is "resolvable + active", so a Mist pauses their one onboarding walk. `ds3_lifecycle_member_departed` and `ds3_lifecycle_group_closed` re-issued in place, byte-identical except three predicates widened to `status in ('active', 'paused')` (extracted from `20260719190205` at authoring, not retyped; the migration self-verifies the two contracts and 1 + 2 widened predicates; the wielded-exit branch already reached paused rows and is untouched). Manifest: both contracts under DS-3. **Red-first:** `journey-enrolment-pause-contracts.test.ts` — 11 cells red at HEAD (PGRST202) and the two cascade cells red for the right reason (a row paused by admin SQL stayed `paused` through `leave_group` / `delete_group`), one labelled PIN (the direct UPDATE refuses at the grant — existing posture, not this task's); `function-classification-completeness` red at HEAD on the two registered-but-unapplied contracts. Both flip green at apply. The step contracts (`enter_journey_step`, `complete_journey_step`, `save_step_response`) refuse a paused walk P0001 with no change — pinned here and already in the completion-timing / response-capture suites. ADR-U047 rule 7 flagged for amendment at the gate (an ADR edit is a carve-out).
