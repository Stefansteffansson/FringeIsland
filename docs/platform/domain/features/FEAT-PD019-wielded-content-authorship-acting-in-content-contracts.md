...
**Corrective note 2026-09-02 ([TASK-ANN-01](../../../planning/backlog/tasks/TASK-ANN-01-admin-suspended-announcement-sight-regression.md)).** This tranche's re-issue of `get_group_announcements` **dropped the FEAT-PC026 admin sight arm** from the personal branch (the `OR (is_platform_admin() AND suspended)` clause), refusing platform admins on suspended groups' announcements for thirteen days. The tranche's sibling sweep named only the announcement suites; `suspended-group-admin-access.test.ts` names the function too and was not run. Restored byte-for-byte by migration `20260902220000`; the wielded limb is untouched. The T1/T2 re-issues (forum, conversations) kept their arms — checked at the same pass.
...
- **Walk lesson (recorded 2026-08-20, the rider's cause):** the T2 mechanism walk enumerated a hand-picked function list and missed `leave_group_conversation` — the seventh family member — caught only when the Hub half's Leave door had nothing to call. **A family walk sweeps the family** (`pg_proc` LIKE '%conversation%', not a curated list); misses found at the consumer build are decomposition faults, not build riders.
