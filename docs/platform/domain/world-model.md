...
- **Q4 — The World → Arc NPC promotion seam.** Who may add the character layer to a world-placed NPC, and how the handoff is recorded (the inter-studio collaboration protocol is an open thread per the bible's beings chapter, S30). Joint question with DS-2's descent. **Resolved 2026-06-10 (DS-2 descent, ratified by Stefan):** a **Teller** promotes (PC-3 `has_permission()` against the Teller role template — the Arc Studio entry check, ADR-U026); the promotion is recorded as a **DS-2-owned character-layer row plus promotion record referencing this service's NPC world-layer row by ID** — additive, no DS-1 write, no DS-1 schema change; DS-1 remains unaware of which NPCs carry character layers (DS-2 consumes DS-1, never the reverse). The wider inter-studio collaboration protocol (S30) remains open as DS-2's §8 Q6, refining at studio decomposition. See `narrative.md` (§2, §3, §L3 Sources-status).
- **Q5 — Lore's shape.** Registry of world-facts in DS-1 vs narrative content in DS-2/DS-4. Thin row kept in the inventory; shape resolves on first demand. **Resolved 2026-06-10 (DS-4 descent, ratified by Stefan, joint with DS-2 §8 Q3): lore is a DS-1-owned registry of world-facts, thin-with-shape-named — registry rows whose renderable presentation, where one exists, is an opaque DS-4 content-block reference (the standing reference direction holds: this service never calls DS-4). Story-canon stays DS-2's and everything renderable is DS-4's, per the three-way rule. The Lore registry capability row stands unchanged. See `narrative.md` §8 Q3, `content.md` §1 and §L3 Sources-status.**
- **Q6 — Place 3's name.** Deferred in canon (the bible's worlds chapter); a naming-register item, not a DS-1 blocker.
- **Q7 — Salience-channel input contract.** The exact shape of DS-7's maturity feed onto the cord (push event vs derived read) — settled at DS-7's descent against the Whisp-split decision (PENDING.md). **Resolved 2026-06-11 (DS-7 descent, ratified by Stefan): push event — DS-7 calls this service's salience intake contract with derived salience; a derived-read shape would have DS-1 reading DS-7, inverting the settled dependency direction (the world never depends on the intelligence). See `intelligence.md` §8 Q3 and §L3 (Salience derivation & feed).**
