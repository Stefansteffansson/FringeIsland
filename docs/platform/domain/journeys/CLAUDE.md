...
- **Loop runtime state and per-FIM pacing are DS-3's (resolved DS-2 §8 Q1+Q2, 2026-06-10)** — and with them the personal-data weight: per-traveller runtime state, persisted loop outcomes, and personalisation state are FIM/Mist personal data; Mist-generated state inherits ADR-U031 TTL-erasure.
...
- **Transcendence migrates journey state.** A Mist's enrolments/progress/loop state are "the session's experience" under ADR-U031's atomic, nothing-restarts migration — any new Mist-touching DS-3 table joins that cascade spec (ADR-U016, jointly with PC-2) before build.
...
- **Ground truth:** the bible's growth chapter (the three questions, the two ways of engaging, what the world may see of you); the bible's story chapter (respawn section + journeys line); the bible's worlds chapter; the bible's roles and governance chapter (Wayfinder; Steward/Guide/Participant/Observer); universe-discovery S19 (signature vs charter).
