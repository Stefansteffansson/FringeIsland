...
| Group-scoped forum structure (threads, posts, replies) | [FEAT-PD009](./features/FEAT-PD009-forum-and-attribution-contracts.md) | 5-in-cycle | A-COM Cycle C-B; contracts over the D15 substrate, write-narrowed; flat threading stays trigger-enforced |
| Membership-status attribution display (ADR-U021) | [FEAT-PD009](./features/FEAT-PD009-forum-and-attribution-contracts.md) | 5-in-cycle | The COM-14 ladder ("Former member" per CB-9, 'Unknown' fallback); applies to forum + conversation detail; display law, never data mutation |
| Community-scoped moderation surface | [FEAT-PD009](./features/FEAT-PD009-forum-and-attribution-contracts.md) | 5-in-cycle | **Partial:** soft-delete moderation contract only; reports store + queue defer to C-D (CB-4) / A-ADM |
| (remaining eight) | — | — | No FEAT-PD specs yet; realized notification code predates this partition (see §L3 Step 2). A-COM cycles C-C..C-E spec live delivery (U039), routing slivers, and lifecycle cascades + export; feeds, cord-health glance, journey-scoped surfaces, attachments, and preferences stay forward (A-NTF / A-DIS / later). |
