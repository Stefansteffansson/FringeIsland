...
| AC3-O6 (D4-5) | **CLOSED 2026-07-31 (Cycle ADM-A).** The durable binding exists: FEAT-PC019's `record_auth_event()` (migration 20260731190000) + FEAT-H034's wiring of all four callers (awaited-but-non-fatal; farewell persists before the erase; sign-in email kept out of the durable row; the `A-OPS` naming and the TODO gone). Live action strings verified at wiring: `auth.sign_in` / `account.created` / `identity.transcended` / `mist.explicit_erase` — this row's earlier caller-name prose was partly off the code. | producer suite 5/5; E2E end-to-end row proof |
| AC3-O7 (D4-6) | **CLOSED 2026-07-31 (Cycle ADM-A).** The sink exists: ADR-U052 + FEAT-PC018 (`telemetry_events`, deny-all; never-raises recorder; 90-day prune; admin-gated statistics read — migration 20260731180000 applied). The emit-without-store state is over; remaining breadth (398 BFF emission sites not yet on the durable leg) is enumerated in FEAT-H034's implementation notes with the adoption-criteria question routed to the area gate — a recorded partial, not a silent cap. | observability suite 10/10 post-apply |
