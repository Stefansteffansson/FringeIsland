...
- **AC5-O2 — the `cycle-current.md` convention has never been used.** `docs/planning/cycles/` holds only a README; every cycle since June ran as an area completion plan or a dated COR plan under `docs/planning/hub-v2/`. PROCESS.md §3 and root `CLAUDE.md` still describe the file. → **R-14** (retire the convention in PROCESS.md, or adopt it for the post-Ferd cycles).
...
**R-14 — the cycles convention** (from AC5-O2). PROCESS.md is a steering file; retiring or reviving `cycle-current.md` is Stefan's call. **Recommend retire**: name the dated plan documents under `docs/planning/hub-v2/` (and their successor directory after Ferd) as the cycle plans, and drop the row from root `CLAUDE.md`.
