...
- **Cycle boundary (~2 hrs):**
  - **Repoint the front door.** The moment the closing cycle's plan is marked closed, `cycles/cycle-current.md` names what follows — the next cycle's dated plan, or "next: …" — so nobody (Stefan first) has to hunt for the newest date-stamped file. The unit gate `hub/tests/unit/platform/cycle-current-front-door.test.ts` is red until it does. (Adopted 2026-09-05, Audit V R-14; shape in `cycles/README.md`.)
...
- [ ] **Post-apply verification covers the journeys nothing names** — when a migration changes surface-reachable behaviour, the affected E2E journeys join the post-apply verification set beside the sibling-assertion sweep: **run, not re-authored**, minutes per gate. The sibling sweep covers assertions that name the changed object; this covers the journeys that merely walk through it. Adopted at the Platform-Ops area gate (Q1, Stefan, 2026-08-02) after four E2E specs broke silently under COR-C and read as environment faults for a costly while; codified here at the Ferd leftovers pass (2026-09-02). The tier-level statement sits in [`docs/platform/CLAUDE.md`](../platform/CLAUDE.md) with the sibling-assertion rule. The set runs **on the test project first** (ADR-U053 §2: `node scripts/apply-migration.js <file>`, record, the conformance family, the post-apply set); production takes the apply on the named approval only (`ALLOW_PRODUCTION=1 … --production`); `node scripts/migration-drift.js` green closes the gate.
