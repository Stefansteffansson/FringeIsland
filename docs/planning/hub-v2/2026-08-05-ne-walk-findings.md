...
## Post-walk correction (2026-08-06) — the E2E flake family had a second mechanism

The walk's close claimed TASK-E2E-01 retired on the strength of the profile.spec fix plus two green fleets. The next fleet falsified it (`signup.spec` red, six specs unrun): **`sessions.spec` signed in as the shared identity in its own fresh contexts and revoked a non-current session of that same shared user** — sometimes the storageState one. The audit that cleared it had discriminated on *browser context* rather than *identity*; fresh context ≠ fresh identity. Fixed to a dedicated FIM (4/4), **fleet 133/133 leak 0→0** after. Full correction, the new rule, and the deliberately *uncleared* remaining audit scope live in [TASK-E2E-01](../backlog/tasks/TASK-E2E-01-profile-shared-session-flake.md). None of it touches the walk's product verdicts — every WA finding and every scenario result above stands as recorded.
