...
**Walk observation (not a rider) — flat 2-level threading is right, but the affordances that make it livable are missing.** Scenario 5 raised the question "why only 2 levels, is that normal?", and the answer matters for the next pass. Two findings. **(1) The invariant has no recorded rationale.** `enforce_flat_threading` pins the depth in the substrate with an oracle-pinned message ("Cannot reply to a reply. Maximum thread depth is 2 levels."), carried in from the v1 build as a strong oracle invariant (B-COMM-004..007) — preserved, never re-argued. The specs record the constraint and its consequences (PD009 §68: "flat threading bounds depth, not width") but nowhere the *why*. An invariant this visible should carry its reason; a walker asked the question the docs couldn't answer. **(2) Stefan's Facebook evidence names the concrete target.** Facebook is structurally the *same* two-level model — a reply to a reply lands in the same flat list, not a third indent — and it works because of two purely surface affordances FringeIsland lacks: replies **collapsed by default** behind a `View N replies` disclosure (that is what keeps a long thread scannable, and it is the real answer to the width bound PD009 §68 accepted), and an explicit **address** — the composer reads `Reply to <name>` and the reply body opens with that person's name as a link. Note what this implies: addressing needs **no schema change and no weakening of the depth invariant**, because the address is *content*, not structure (a prefilled mention), and collapse is pure rendering over the existing unpaged reply payload. So the forum's next pass has a precise, cheap target rather than a vague "make threading better": keep the two-level law, add collapse + addressing. Carried to the retro with the titleless-thread question above.
