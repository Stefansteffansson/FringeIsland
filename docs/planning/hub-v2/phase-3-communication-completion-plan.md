...
- [x] ADR-U043 measurement pass (cold ≥20-min idle ×3 + warm, tail rule) + Stefan's live walk — both before the area retro *(measurement DONE 2026-07-21/22: warm all-PASS, three deep-cold windows run, W3 tail-FAIL dispositioned as labelled exception with the never-skip-the-pass rider; **live walk DONE 2026-07-22** — 10/10 scenarios, 4 riders run to ground and merged, 2 wording fixes shipped; **verdict: PASS with riders dispositioned** — see the gate record's verdict section)*
