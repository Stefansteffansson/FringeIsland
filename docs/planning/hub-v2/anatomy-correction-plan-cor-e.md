...
| 4 | **R-14** — `cycle-current.md`: retire the convention (PROCESS.md §3 + root `CLAUDE.md` row) or adopt it for post-Ferd cycles | ruling (steering-file carve-out) | **Retire** — name the dated plan documents as the cycle plans; drop the `CLAUDE.md` row |
...
- Root `CLAUDE.md` header: "Last updated" moves; the "Reflects:" sentence becomes a pointer — *the entity model and decisions are the anatomy's (`docs/architecture/ARCHITECTURE_ANATOMY.md`, stamp inside) and the ADR index's* — no ADR enumeration in the header. If R-14 = retire: drop the "Active cycle" row from the document map.
- `docs/planning/PROCESS.md` §3 (only if R-14 = retire): one sentence naming the dated plan documents as the cycle plans.
...
**Gate:** doc-health §3 pointer integrity; the dashboard regenerates (`npm run dashboard`).
...
**Gate:** the skill's INVOKE record for the next run cites §11's new rows.
...
- The routing chain a new session walks — `SESSION-OPENER.md` → `CLAUDE.md` → `AGENTS.md` → `hub-v2/README.md` → the latest bridge → `waves/ferd.md` — reads true at every hop on 2026-09-XX.
