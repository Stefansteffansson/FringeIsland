...
- **A single-ping keep-warm cannot fully fix this** — it holds exactly one instance; the first real fan-out sails past it. §6 finding 3 ("L2 is the primary fix") is downgraded to *partial*: it removes the sequential first-visitor wave only. **L2′ — concurrent multi-ping** (each tick fires ~4 concurrent requests to hold a pool) is the candidate stopgap; validation experiment (ticks at 4.5-min gaps, then a 4-way page-boot burst) run this session — result recorded in the session bridge.
