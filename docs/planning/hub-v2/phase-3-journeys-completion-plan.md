...
|-------|--------------|----------------|
| **J-A — Catalogue & enrolment** | JRN-1, 2, 3, 4 + GRP-4 enrolment-summary slot fill | Platform half: DS-3 catalog-read + enrolment-lifecycle contracts (the first FEAT-PD spec) over the existing RLS + permission keys; additive enrolment-summary on `get_group_detail` (the G-A seam). Enrolment contracts carry the ADR-U045 identity-status gate (Mist → designated onboarding journey only; FIM → published journeys). **Kickoff-batch riders on the same schema-gate migration:** `get_my_pending_nominations()` mirroring `get_my_invitations()` (`leadership.ts` becomes a thin consumer — closes the audit LOW finding) + the org-spec §5 seeding-sites staleness fix. Perf: catalogue/detail are B2/B3 surfaces; justified standalone reads + session cache (not overview-bundle slices — navigation targets, not first-paint). |
...
- **J-O3 — measure first, then choose.** The ADR-U043 area-gate waterfall runs on production before the bet is chosen; a budget miss requires ≥ 1 felt-path item. Candidates: P3b journey slice, P1-residual, P4.
