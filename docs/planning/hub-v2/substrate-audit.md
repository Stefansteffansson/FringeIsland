...
| `pending_email_invitations` | PC-3 | **Conformant** | Non-FIM invite-by-email (Hub MEM-2). Group-keyed. *(ADR-U040, 2026-07-05: slated for retirement — referral model; live until the MEM-2 rebuild is scoped.)* |
...
| **Stewardship / succession** | `nominate_steward`, `_handle_stewardship_nomination_action` | **Conformant** | Hub MEM-7 leadership transfer. *(Outcome at G-E, 2026-07-05 — FEAT-PC014: `nominate_steward` **replaced in place** + hardened; `_handle_stewardship_nomination_action` **dropped** with the generic dispatch surface; succession now rides `respond_to_stewardship_nomination` / `hand_stewardship_to_deusex`.)* |
...
| **Notifications / triggers (DS-5 / V3)** | `handle_notification_action`, `notify_group_deleted`, `notify_invitation_accepted`, `notify_invitation_declined_or_member_change`, `notify_invitation_received`, `notify_role_assigned`, `notify_role_removed` | **Conformant** | Event-driven notification fan-out. DS-5/V3 territory (PW-1). *(2026-07-05 — FEAT-PC014: `handle_notification_action` **dropped** — the generic caller-data dispatch was the ADR-U038 hole; the `notify_*` trigger fan-out is unaffected.)* |
