...
- **N-B — Smart (actionable) notifications + dispatch + expiry.** NTF-4/5/6/8. Typed-action UI (Accept/Decline + acknowledge + extensible action registry, no sealed enum); **NTF-6 re-derived per NB-1** — the action submit records `action_taken` and thin-dispatches to the **already-existing dedicated handlers** (`accept_group_invitation`/`decline_group_invitation` — pc012:522,562; `respond_to_stewardship_nomination` — pc014/cor-a; `submit_content_report` — c-d:600); NTF-8 lazy-expiry-on-view (mark expired at read-time, hide action buttons). Paired FEAT-PD### ↔ FEAT-H###.
  - **N-B SCOPE LOCKED (Stefan, 2026-07-24) — settle before N-B decomposes:**
    1. **Schema-gate touch:** extend `get_own_notifications` to carry `action_data` (N-A deliberately withheld `action_data`/`action_taken_at`; the typed-action UI needs `action_data` to render — e.g. the group + the response window). This is the one contract change; the action columns themselves already exist (sprint3).
    2. **Actionable-notification kinds in scope (re-homed into the bell + inbox):** `stewardship_nomination` **and** the group-of-groups **acting-invitation** response (FEAT-PC015). Both are `action_type`-bearing (substrate sets them: pc014/cor-a for nomination, pc015:608 for acting). NTF-5 renders Accept/Decline (+ acknowledge) generically from `action_type`, each behind a `ConfirmModal`; NTF-6 thin-dispatches to the existing dedicated RPCs (`respond_to_stewardship_nomination`, the acting-respond RPC — reuse the existing `app/api/notifications/[id]/nomination-response` route pattern / `app/api/groups/[id]/acting/respond`, do **not** rebuild a generic dispatcher).
    3. **Close the D8 re-home seam:** **retire the bespoke `PendingNominations` section** above `/groups` (its own header names A-NTF as the re-home target) — the inbox/bell becomes its home. Likewise fold the acting-invitation response affordance.
    4. **Out of scope (left bespoke):** group **invitations** stay on the `my-invitations` panel (`/groups`) — `invitation_received` is a *passive* notification, richer in-context, never flagged for re-homing. N-B does not make invitations actionable-in-bell.
    5. Reconcile: acting in the inbox and in any surviving in-context surface must stay consistent on re-fetch (no realtime until N-C — refresh-based).
