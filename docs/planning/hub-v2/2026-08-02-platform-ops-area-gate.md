...
## Measurements (ADR-U043) — EXECUTED 2026-08-02

Full record: [`2026-08-02-adm-gate-measurements.md`](./2026-08-02-adm-gate-measurements.md); ledger row appended. Headline: **cold 3 638 / 4 395 ms** across two ≥22-min enforced-idle windows (provisioning-dominated — the fan-out fires at 2.8–3.1 s; extends the standing labelled pre-launch exception, measurably better than the ~5.5 s A-NTF era) · **warm fresh-context 415–964 ms PASS** on the list/dashboard/queue surfaces · **carried finding for the verdict: the two detail pages cross the 1.0 s B3 ceiling in 3 of 5 fresh-context runs (1 036–1 127 ms, up to 1.13×)** — the A-NTF ceiling-hugging class over the line. Verdict options were accept-as-carried or commission a warm investigation — **CALLED by Stefan 2026-08-02: the warm investigation is COMMISSIONED** (the A-NTF 937 ms precedent — that one closed its finding as not-a-defect by measurement; this one starts from three actual crossings). Scope: the two detail pages' fresh-context loads (bundle + hydration + the ~500 ms "unaccounted" slice; the API reads are already fast). **First item of the gate-close session.**
...
**HELD.** The measurement pass landed 2026-08-02 (ledger row appended) with one carried finding whose disposition is CALLED: **the warm investigation is commissioned** (Stefan, 2026-08-02). The gate closes in the next session, in this order: **(1)** the commissioned warm investigation (detail pages' fresh-context ceiling crossings) · **(2)** the deferred-five per-row calls · **(3)** the two process-question verdicts · **(4)** Stefan's live walk. Then: the area retro (ADM-C + ADM-D fold in), the task sweep, the owed doc-health-check (cycle boundary), and the **AB-6 FULL anatomy audit** opens as the Phase-4 cutover's entry condition.
