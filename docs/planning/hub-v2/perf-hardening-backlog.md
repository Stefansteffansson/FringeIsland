...
- **Boundary-bet rule (amended 2026-07-07, [ADR-U043](../../architecture/decisions/ADR-U043-performance-budgets.md) / Groups retro §4):** the bet is chosen **after** a fresh real-path measurement at the boundary (the ADR-U043 area-gate waterfall), and when that measurement shows a budget miss the bet must include at least one **felt-path** item — never only DB-layer hardening. (The Groups boundary executed P3a faithfully while the felt regression lived in the client fan-out; measure first so the bet aims at the layer that is actually slow.)
