...
**Status:** Closed — all findings resolved or routed (2026-06-23) · **Opened:** 2026-06-22 · **Reviewer:** Stefan
**Resolution log (2026-06-22):** **F-04 applied** — SPECIFICATION §L3 (A-COI intro + 7 COI rows + 2 external-dep rows + Sources-status provenance entry), DESCRIPTION, OPEN_QUESTIONS (CQ-009 note), and both tours (HUMAN Ch. 6 + TECHNICAL COI section/table/deps); the "Mentor" *entity* is retired and recast onto the dialogic, AI-driven Whisp (mentoring kept as a *function*). See [`F-04-whisp-amendment-proposal.md`](./F-04-whisp-amendment-proposal.md). **F-02** (Ch. 1 GDPR hard-delete) and **F-03** (Ch. 3 player) fixed in HUMAN.md — committed with F-04 (a34f87b). **F-01 done** — 8 chapter founding-question closers removed + Epilogue reworked in HUMAN.md (TECHNICAL.md "Founding Q" column left as-is pending Stefan's call, since it mirrors the §L3 column which stays). **F-05 done** — ADM-18 added to §L3 A-ADM (admin removal of member(s) from a specific group or groups; DeusEx override of MEM-5); A-ADM count 17 -> 18; provenance in Sources-status; propagated to HUMAN Ch. 8 + TECHNICAL.md (row, member-ops cluster, auto-routing note, scorecard 105 -> 106). **F-07 framed** — surveyed `universe/` + `thinking/` (grounded against VISION.md); added **CQ-016** (Hub experiential trajectory across waves + whether the DESCRIPTION conveys the ambition) to OPEN_QUESTIONS. Stefan confirmed the first Hub build is deliberately platform-fundamentals — sequencing, not omission — so the forward experiential scope is routed to a **discovery session**, not resolved here. (Aside: confirmed the Journal *is* specified — IDN-5, Partial-Ferd, build-new; present in HUMAN Ch. 1 — kept as-is.) **All Phase-1 review findings now resolved or routed.**
