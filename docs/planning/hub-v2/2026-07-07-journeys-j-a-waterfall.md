...
**Boundary bet: DECIDED — P1-residual** (Stefan, 2026-07-07, same session as the measurement): the sign-in landing's `/api/me/overview` bundle (~870 ms of the 1 375 ms click-to-content) is the target. P3b stays parked (its DB-layer condition not met); P4 unargued-for by the data.

**CLOSED same day (Stefan, "close it"): measured to the floor.** The step-1 instrumentation (below) showed the app share already at its bound and the remaining terms vendor/network/once-per-session boot; the amendment path buys ~60 ms; the login-page warm-up experiment declined (complexity for a within-budget number). **Deliverable shipped: the permanent `x-overview-timing` breakdown** (PR #123) — any future regression surfaces with its own diagnosis attached. Exit-checklist disposition: closed with evidence, never silently dropped.
...
### Step-1 instrumentation result (same day, PR #123 — `x-overview-timing`): the theory above is REFUTED

Paired browser/server runs on production (5 warm + 1 cold, Stefan's session): **the function is near the floor** — warm in-function total **51–73 ms** (auth 1–5 ms; the five slices overlap, slowest 48–68 ms), cold first-invocation in-function **243 ms**. The browser-observed 177–387 ms warm / ~1.4 s cold therefore live **outside the function**: a **~120–330 ms per-request client↔edge network constant** (TLS + routing + distance — applies to every route equally) plus, on the first hit of a session, the **isolate boot** before the function's first line. Consequences: (a) the five-round-trip theory is dead — the parallel slices share connections fine; (b) the `get_own_overview()` substrate RPC would save ~60 ms — **the ADR-U042 amendment is NOT justified, guardrail 4 stands**; (c) the bundle is already doing its job (one network toll instead of five). The only app-controllable residual is the once-per-session cold boot (~250 ms in-function + isolate spin-up); everything else is vendor auth floor + network physics. J-O5 measured 1 375 ms against the 2 000 ms budget with the app share at its ~1.0 s bound — **the residual headroom is small and mostly not ours.**
