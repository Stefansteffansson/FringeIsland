...
**Post-merge sibling correction (2026-09-03, found by INT-01's two full runs — the same four cells, both runs):** my sibling sweep grepped member-client writes on the four tables that carry write policies only. Four ADR-U038 direct-caller cells on tables with *no* write policy asserted RLS's silent "error null, 0 rows" — `forum-contracts` (own-post edit; Steward moderation), `conversation-contracts` (own read cursor), `notification-contracts` (own `is_read`) — and now see the grant's 42501 first. Adapted with labels (same outcome, earlier lock). **The honest sweep rule, recorded:** for a grant change, grep every member-client `.from(<any table>).insert|update|delete|upsert` across all suites, not the policy-bearing tables — a write that RLS filtered silently is exactly the write a grant revoke turns loud. One transient in run A: `account-state-read` afterAll timed out at 30 s in `cleanupTestUser` (the Management-API throttle tail, memory `management-api-throttle-mass-red`); absent in run B; not this task's.
