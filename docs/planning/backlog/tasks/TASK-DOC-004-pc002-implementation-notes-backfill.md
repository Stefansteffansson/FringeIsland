...
**Duplicate closed.** This exact finding was re-filed on 2026-07-24 as `TASK-DOC-006-pc002-implementation-notes.md` after the A-NTF N-B boundary re-found it — the audit did not check the open backlog before writing the finding. DOC-006 has been deleted as a duplicate of this task. The process fix shipped in the same PR (#280, merged 2026-07-25): `doc-health-check` Section 5 step 6 is now a whole-tree sweep **carved out of the section's skip clause** (the skip clause was the root cause — an unchanged `6-done` spec could never be re-scanned), findings are diffed against the open backlog before filing so a re-find escalates the existing task instead of minting a new id, and `PROCESS.md` §3 gained a cycle-boundary backlog-triage step.
