...
| PC-1 Infrastructure | **Next** | Finding #3 reframing per PC-4 §L3 Step 2 C2-5 + C3-2 (three-pattern audit-write codification: SECURITY DEFINER direct-INSERT + SECURITY DEFINER trigger-mediated + anon-key client RLS-gated INSERT) + Finding #4 two-tier centralization PC-4-scope anchor (Gap A substrate + Gap B auth-flow plumbing; concrete fix candidate `lib/supabase/server.ts` admin-tier helper introduction; helper-introduction disposition deferred — write at amendment OR route to FEAT-PC-* feature spec) + X5 two-tier centralization reframe per PC-3 §L3 Step 2 C3-2. Phase 2 close-out (commit `8976646`) routed both PC-4 findings to this amendment-list. | After Phase 2 close-out | (to author) | (to author) | (pending) | (pending) |
