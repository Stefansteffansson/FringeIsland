...
## Part 2 — the MAINTAIN ruling, same day (TASK-SEC-04)

Stefan, on SEC-03's deferred item 1: *"revoke MAINTAIN then, same gate shape."* Measured first (both projects identical, PG 17.6): `anon` holds MAINTAIN on 37 tables, `authenticated` on 39; the default ACL's `rm` carries it; no information_schema view shows it, which is why SEC-02 never saw it. Built the SEC-02 way, red first: the new `aclexplode` cell in the lockdown gate (76 rows at HEAD) and the `m` letter in the default-ACL cell (`anon=rm`), the static gate's MAINTAIN fixture (1/10 red → 10/10); then the migration `20260923150000_task_sec04_client_role_maintain_revoke.sql` — a DO-loop revoke + the default-ACL revoke + a self-check — rehearsed inside a rolled-back transaction (client-role MAINTAIN 0, `service_role` 42, the standard-grant hash untouched). [`TASK-SEC-04`](../backlog/tasks/TASK-SEC-04-maintain-off-the-client-roles.md) — **"ok merge" given the same session.** Gate record: test leg applied + repaired; `test:integration:platform` **49/49** (the lockdown gate 7/7, red 2/7 an hour earlier); production leg applied + repaired; drift files = test = production = 144; read back on both projects — client-role MAINTAIN **0**, `service_role` 42, default ACL `anon=r, authenticated=r`, the standard-grant md5 `8ce5c98e…` unchanged, version recorded. The live projects now equal the chain. #684 merged.

Open after this: `anon` SELECT on 35 tables (SEC-03's item 2) — needs the anon-policy audit first.

_Doc health, Part 2: the delta is one migration (grants only) + one task file + its index row; every relative link in the six touched planning files resolves; the front door still names this bridge; nothing else triggered. One line, not a block._
