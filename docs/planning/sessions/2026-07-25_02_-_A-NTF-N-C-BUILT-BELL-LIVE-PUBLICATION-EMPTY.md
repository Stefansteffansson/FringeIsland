...
- [x] **FULL integration sweep — COMPLETED after this bridge was first committed: 663 passed, 3 failed (55 suites).** Recorded here rather than left as a forward reference. **None of the three were caused by N-C**, and each was ruled out structurally rather than by assertion:
  1. **`FEAT-PD013` STORY-2 — N-B's leftover, now FIXED (2026-07-26).** The assertion pinned the *pre-N-B* payload key set; its own comment read *"action_data/action_taken_at deliberately excluded until N-B names their consumer"*. N-B added `action_data` to `get_own_notifications` on 2026-07-24 and did not adapt this sibling, so it had failed ever since. Verified against the deployed contract (13 returned columns; `action_taken_at` correctly still withheld), fixed, and the constant renamed off its cycle label — an "N-A keys" list holding an N-B key is a small lie. **Suite now 16/16.** *This is the red I earlier misattributed to the Supabase ES256 incident: it was this all along, and the misattribution cost part of a session.*
  2. **`FEAT-PD011` STORY-6 — structurally impossible for N-C to cause.** It counts rows scoped to `topic = 'group:<id>:forum'` (`window-and-report-contracts.test.ts:347,355`); N-C's trigger only ever emits on `account:<auth_uid>:notifications`. Not diagnosed further — filed.
  3. **`FEAT-PD008` RIDER-1 — no established connection** to notifications or realtime (a `create_group_conversations` permission-backfill invariant). Not diagnosed further — filed. **Note:** if it is a real backfill gap rather than a stale assertion, it is user-facing (members unable to create group conversations), which is why the filing is not `low` priority.
  - Findings 2 and 3 are filed as **`TASK-INT-02`**, to be done **before N-D's own sweep** — with known reds in the suite, nobody can use "is it green?" as a gate, and that erosion already cost this cycle real time.
  - **Process finding for the A-NTF area retro:** N-B changed a shipped contract's payload and left a sibling assertion pinning the old shape. The house rule that a schema-gate PR changing shipped semantics **budgets sibling-suite adaptation as in-scope work** was missed — and the resulting red then masqueraded as an environment fault for two days.
