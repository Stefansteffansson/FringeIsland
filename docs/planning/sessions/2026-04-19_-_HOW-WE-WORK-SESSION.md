...
- **Three research mechanisms exist but aren't unified.** Maturity-2-explored, `type: spike` with template, and `docs/fringeisland-thinking/README.md` for long-form reports. Stefan's question about bucketing / research / backlog surfaced the missing connective tissue: when does a question warrant spike vs report? How do findings flow back into affected specs? G-18.
