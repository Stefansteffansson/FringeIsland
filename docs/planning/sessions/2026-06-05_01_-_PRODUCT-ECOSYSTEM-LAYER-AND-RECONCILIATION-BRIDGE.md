...
**Precedence - the discovery is the single source of truth.** The universe-discovery work (Statements 1-46 + the product/ecosystem locks) outranks every other statement anywhere in the repo. Where any file - canonical docs, ADRs, specs, or code - conflicts with the discovery, the discovery wins and that file is what gets corrected; the discovery is never bent to fit existing text. This elevates the discovery from its `thinking/` status to the reconciliation authority - graduation flows from the discovery outward to the rest of the repo. The rule fixes the direction of resolution; ratify-before-edit still gates the actual changes to ADRs, specs, and code.

**Validation is repo-wide.** Session B validates the WHOLE repo against the discovery - all relevant files, docs and code, not a shortlist. CC may own parts or all of both the Session A inventory and the Session B conformance passes (dual-sweep: CC executes the repo-wide walk, grep, and per-file conformance read; Claude.ai adjudicates each against the discovery).

**Session A - the map.**
- Scope by purpose: inventory only files that assert what FringeIsland *is* or how it *works* (universe, products, roles, cosmology, platform behaviour). Skip pure process/tooling/planning unless it describes the thing. This surface is broad and spans altitudes - the vision/why, the universe fiction, the architecture (anatomy + the ADRs), and the capability specs and L1->L3 derivations that describe how it works in detail (the descent already done is itself part of the description) - far more than the obvious VISION / manifesto / Hub DESCRIPTION, and the surface is the WHOLE repo - docs and code alike, since the codebase asserts how the platform works and must conform too. Naming the actual set is the point of Session A; do not pre-decide it.
- Bootstrap with CC, not memory or `search_files`: have CC emit (a) a full repo tree (docs and code, excluding build and dependency directories) and (b) a repo-wide grep for a fixed term list (Hub, Gimbal, Game, studio, village, safe harbour, console, Shadow, FIM, Whisp, ball, affordance, Dreamineer, DeusEx, transcend, anchor/seed, portal). CC may own parts or all of the walk; Claude.ai organizes and adjudicates the raw output against the discovery.
...
1. **A ratified, reconciled description** of what FringeIsland is and how it works - divergences resolved into one coherent picture, each marked graduate-now or defer. Canonical edits happen only on ratified items, at graduation, and only to the files Session A's map identifies as carrying the description - expected to be many more than the obvious VISION / manifesto / universe / Hub DESCRIPTION (the architecture anatomy and the ADRs, the ecosystem universe/ files, the feature specs and L1->L3 derivations, the AGENTS / tier CLAUDE.md structure docs, STATUS.md, and the studios/products descriptions among them). The session does not rewrite canon from not-yet-ratified discovery. Whether this lands as one description doc or as the existing set is decided by Session A's canonical-description finding.
2. **A repo-wide conformance register** - every relevant file in the whole repo (docs and code) checked against the discovery: conforms / stale / contradicts / missing, with the resolution being correct-the-file-to-the-discovery (or fill-the-gap), each marked graduate-or-defer. Its architecture and capability view is the **descent-impact register** that feeds the later validation of the L1->L3 descent: for every reconciled point bearing on the architecture, the capability area it touches (PC-1..PC-4, DS-1..DS-7, a vertical, the Products / Platform API layer) and whether it **confirms / revises / adds-to** that capability, split into:
...
**Net chain:** Session A (repo-wide map) -> Session B (discovery confirmed as the single source of truth + reconciled description + repo-wide conformance register, with the descent-impact register as its capability view) -> later validation/correction of the whole repo, and of the descent, against the discovery (re-check PC-1..PC-4; constrain DS-1..DS-7; conform docs and code).
