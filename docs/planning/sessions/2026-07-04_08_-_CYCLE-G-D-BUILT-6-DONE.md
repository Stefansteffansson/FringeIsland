...
4. **Launch checklist (planted 2026-07-04, post-close manual testing; Stefan):** (a) **custom SMTP before any cohort onboarding** — the built-in provider caps ALL auth email (confirmations/recovery/magic links) at 2/hour project-wide; dovetails with the D4/V3 outbound-dispatch seam at the Notifications area (one plumbing decision covers both). (b) **Per-IP sign-in headroom for venue events** — Supabase Auth sign-ins are IP-token-bucketed (~30-request burst per IP), so a workshop cohort behind one venue Wi-Fi shares a single bucket; raise the configurable limits ahead of known events and prefer "sign in beforehand" guidance (sessions persist). Dev limits raised 2026-07-04 via the management API (token_refresh 150→600, verify 30→100, anonymous 150→300) after automated-suite traffic locked out manual testing.
