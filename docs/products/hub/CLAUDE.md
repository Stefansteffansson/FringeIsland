...
- **Business logic lives in `app/api/...` route handlers, not in server components.** Next.js App Router's data-in-RSC pattern is tempting because rendering and data are colocated — but every business rule, validation, and side effect must live behind the Platform API so iOS and Android inherit them on day one. Server components render against data that was already fetched (typically client-side via the API); they don't author logic and they don't fetch from the database directly.
...
- **Tier file (read first per load order):** [`../CLAUDE.md`](../CLAUDE.md) — every products-tier rule (API-first, equipment-keyed feature placement, paired specs, Shadows-before-FIMs, permission resolution via `has_permission`) applies here without restatement.
- **Sibling product entity:** [`../gimbal/`](../gimbal/) — the senses surface (planned, not Next.js).
- **Relevant ADRs:** U004 (anonymous sign-in — the Shadow) · U007 (three-layer permission model) · U009 (API-first, frontend-agnostic) · U025 (products as equipment profiles) — these constrain the Hub's auth, permissions, and data-access shape directly.
