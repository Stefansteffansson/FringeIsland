...
- **Mutations of server-confirmed client state write the confirmed response through to the session cache.** "The page owns optimistic state" is scoped to genuinely optimistic progress; anything the server confirmed (a toggle, a completion, a membership change) updates its session-cache entry from the confirmed response in the same handler — otherwise the next client-side mount replays the pre-mutation slice (bound at J-D; the stale sharing-toggle fix, PR #146).
- **A boot/arrival prefetch trigger matches exactly the pages that consume the slice.** An adopted slice is a read that already happened — arming adoption (`OverviewBoot`'s `BOOT_PATHS`, `OnboardingArrival`'s `ARRIVAL_PATHS`, and successors) on a page that won't consume it leaves a consume-once slice lying in wait to serve stale data to a later mount (bound at J-E; the #166 boot-scope fix + the pre-merge `ARRIVAL_PATHS` fix).
...
- **Every API route takes the platform-default Node runtime — no per-route `runtime` or `preferredRegion` exports ([ADR-U036 Amendment 2](../../architecture/decisions/ADR-U036-edge-runtime-hot-read-routes.md)).** The 2026-07-01 Edge era (hot reads on Edge V8 isolates, per-file `dub1` pins) ended 2026-07-10: Vercel deprecated the Edge runtime, measurement showed deep-cold provisioning is per-instance and runtime-independent, and Fluid's in-instance concurrency — the mitigation for the measured concurrent-fan-out boot lottery — is Node-only. The region pin lives in `hub/vercel.json` (`"regions": ["dub1"]`) **alone**, preserving ADR-U035 co-location with the Ireland DB. A runtime or region export reappearing in a route file is policy drift — the route-policy conformance test (`hub/tests/unit/app/api/route-policy-conformance.test.ts`) fails red on it. The old Edge-safe import constraint is gone; the ADR-U037 identity split (`getUser()` on mutations, `getClaims()`/`getVerifiedUserId()` on reads) is unchanged and enforced by the same test.
