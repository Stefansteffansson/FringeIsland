...
> **Amendment (2026-07-18, Cycle J-F / FEAT-H024 STORY-6).** The route additionally composes the FEAT-PD007 walks export (`get_own_step_instances_export()` — the caller's journey enrolments with their own step passages and responses) as the additive top-level `journeys` key, the FEAT-H011 pattern verbatim (present-and-empty for a walk-less member; a walks failure fails the whole download; the courier renders nothing of it). This **discharges the step-instances flag owed since the J-C retro** — the new personal-data category (per-traveller step-instances, now carrying response content, the most personal data in the system) is under right-of-access from the moment the first response can exist (ADR-U046; FEAT-PD007 JF-6). Existing sections arrive byte-identical.
