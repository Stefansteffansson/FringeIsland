...
- **Cold spot-check (ADR-U043 Amendment 1):** this feature adds a request on the arrival first-paint path, so it runs one deep-cold spot measurement of the landing before `6-done` (idle depth recorded, keep-warm pinger paused).
