...
- **The forum tenant — page-scoped.** Mounted with `GroupForumSection` for group G, subscribed to `group:<G>:forum`, torn down on unmount (navigating between groups swaps subscriptions). On a hint: drop G's forum cache (`dropGroup`) and re-read the section's loaded window — new threads appear, moderation tombstones materialize, and (RIDER-3 amendment, A-COM walk 2026-07-22) **own-window edits land live too**: the tenant subscribes to `forum_post_edited` (PD010 RIDER-3 trigger) alongside the two C-C events — while **composer and reply drafts are preserved** (a refresh never eats a member's half-written post).
...
Built red-first at the unit tier over the FEAT-PD010 substrate; carries no migration of its own. PR #218 (merged after the #217 gate apply); the channel-scope steering batch rode PR #215.

- **The substrate** `hub/lib/realtime/` — `manager.ts` (one socket over the AuthContext client; declarative tenant registration; `setAuth`-before-join; `private: true` always; exactly-one-subscription semantics; channel status + rejoin surface via `subscribeStatus`; total teardown; FIM+session arming rule; content-free telemetry hooks), `conversations-tenant.ts` (hint → `invalidateMessagesCache()` + `conversationsChanged` window event carrying the hinted id), `forum-tenant.ts` (page-scoped; `dropGroup` + re-read), `use-comm-channel.ts` (the shared STORY-6 reconciliation hook: status-derived quiet indicator — shown only after a subscribed channel degrades, no first-connect flash; reconcile on recovery + visibility regain; `COMM_POLL_MS` 60s only while degraded + visible, the session-guard constant mirrored), `components/ui/ReconnectingNotice.tsx`. Wired via `useRealtimeTenants` beside `useSessionGuard` in AuthContext; **`session-guard.ts` untouched** (the No-go held); **A-NTF readiness proven by a synthetic-tenant registration test** (the manager needs no edits for the bell to join).
- **Consumers** — `MessagesLink` (badge re-derives on `conversationsChanged`; initial read reshaped to an inline promise chain for the `react-hooks/set-state-in-effect` rule), the `/messages` inbox (background re-read, prior content stays rendered — no skeleton flash), `/messages/[id]` (targeted re-read through the page's existing load path — read-marking byte-identical; foreign hints leave the detail undisturbed), `GroupForumSection` (subscription bound to mount + group id; drafts preserved; tombstones on moderation hints). **Deliberate simplification (in the refetch-don't-patch fence):** a forum hint re-reads the first page; loaded-earlier pages collapse (load-earlier re-clickable, drafts preserved) rather than adding merge plumbing.
- **Red→green, honestly (unit tier, per suite):** manager module-absent → 11/11 · conversations-tenant module-absent → 5/5 · use-comm-channel module-absent → 6/6 · MessagesLink +1 red driver → 7/7 · inbox +2 red → 4/4 · detail +2 red → 6/6 · GroupForumSection +4 red → 12/12. One vacuous-green caught at authoring (the forum foreign-hint no-op) and refitted with a real red driver per the green-at-red discipline. Two `react-hooks/set-state-in-effect` violations fixed cleanly, never suppressed.
- **E2E (`realtime.spec.ts`, 2 journeys — labelled journey verification; the red-first proof lives at the contract + unit tiers):** live DM (B parked on `/messages` sees the conversation rise + unread + badge without navigating; B's open detail then shows A's second message live) and live forum (B's parked group page shows A's new thread with B's half-typed draft intact, then the moderation tombstone live); `bringToFront` deliberately arms both the hint and the visibility-reconcile paths for determinism (doctrine rule 6). **Build finding:** the spec initially rode the shared storageState and failed under fleet ordering — the documented FEAT-H012 suite-order trap (an earlier spec's scope-global sign-out revokes the shared session); fixed to fresh logins for both contexts, after which the spec is green standalone, green under `--repeat-each=3`, and green under the adversarial `messages+forum+realtime` ordering.
- **Gates:** unit **107 suites / 791 tests** · lint 0 errors · `next build` clean · route-policy conformance green (zero new routes, zero new exceptions) · E2E fleet **70/71** with the single failure fenced **found-not-caused** by name: `profile.spec` STORY-4, the standing flake-watch item — proven realtime-independent at this cycle (identical failure with the C-C tenants disabled; two consecutive fresh isolated runs 3/3 green; root cause now diagnosed and routed to `TASK-E2E-01`); the sibling fleet run the same hour completed all-green. Integration comm slice **54/54** post-apply (platform-side; see FEAT-PD010); full integration sweep **46 suites / 534 tests, 533 green in-sweep** with the two non-comm items dispositioned found-not-caused by name: `own-profile-contract` (suite-level afterAll cleanup timeout — the C-B-documented Management-API infra-flake class; green on isolated re-run) and `membership-lifecycle`'s paused-row search test (root-caused: 9 stale fixed-name fixture users — oldest pre-C-C — crossed the capped typeahead's limit-8 window; debris swept via the house cleanup path and the suite hardened with a **single-token, run-unique** fixture name as a labelled adaptation — a first two-token attempt broke the nickname-rule name-sync and was caught and corrected — now **24/24** green; neither item touches C-C code).
- **Performance disposition (ADR-U043):** no first-paint request added or rerouted anywhere — channels join post-paint; hint-driven re-reads are background with prior content rendered (B6 held: skeletons stay first-load only); sends/posts keep their C-A/C-B optimistic behaviour (B5 unchanged). **The Amendment-1 deep-cold spot measurement is not triggered** — recorded here per the DoD.
