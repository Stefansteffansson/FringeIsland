...
### Post-6-done fixes (2026-07-06, Stefan's first manual walk)

- **Revealed visibility — the doors existed, the doorways didn't (finding 3).** After a group was admitted into another group, its wielder had no way to reach the host (private host → P0002; the memberships panel showed its name as plain text), and an invited FIM answered invitations blind. Principle ratified in the fix: *the no-leak rule protects unrevealed groups — an invitation or admission IS the group revealing itself.* Platform (migration `20260706170000`, `get_group_detail` replaced in place): own-**invited** viewers (active groups only) and **wielders of an active member-group** (substitution, ADR-U041 §2a — any lifecycle state, a member's standing) now see the group's **face**; member-list rules and the paused-stays-dark posture (PC013 Open Q3) unchanged; keyless members of the acting group and strangers stay P0002 (guard-tested). Hub: the group name links to the group page in MyInvitations and the wielded memberships panel. **Rider caught by the migration's own verification:** `get_group_detail` had carried **anon EXECUTE since PC010** (one of the twelve documented inert-but-anon-executable contracts) — revoked in place per the touch-it-harden-it pattern; the parked grant sweep shrinks by one. Red-first (2 contract reds + 2 link reds; guards labelled) → unit 411/411, groups domain 187/187.
- **The selector offered hats with no standing.** Every wieldable group appeared on every group page — including the current group itself (acting-as-itself is meaningless, ADR-U041 §2c outward-only) and groups with no membership in the page's group. Fixed three-part: the acting-contexts read became **context-scoped** (`get_acting_contexts(p_context_group_id)` + `is_member_of_context` flag — the paired platform fix, migration `20260706150000`); the page offers only flagged contexts and never the current group, while the unflagged full list still gates the memberships panel; and the acting **empty-state copy stopped inventing a floor** — *"Nya gruppen #1 can view this group"* rendered for a group with zero standing in a private group (false — the copy was the Myself empty-state reused, true only by construction for the viewer). It now reports the substrate's answer: *"{A} is a member here but holds no permission grants."* Red-first on all three tiers (S4b contract red, panel-copy red, route pass-through red) → unit 410/410, contract suite 27/27, journeys 2/2 incl. the new on-own-page one-option assert.

### Walk finding (2026-08-14) — the "use its powers" half is still a seam

Live walk (the Hopper cohort): a wielder flipping the selector on the host group's page correctly sees the hat's powers ("What I can do here" substitution — STORY-1's *see* half), but no content surface consumes them — the hat's `post_forum_messages`/`view_forum` open nothing, because no DS-5 content contract accepts an acting group (platform-side `p_acting` exists only in the role fabric) and no Hub affordance keys on the substitution permissions. Working as this spec scoped it; the STORY-1 intent line ("see **and use** its powers") outruns the build. Registered as [TASK-ACT-01](../../../planning/backlog/tasks/TASK-ACT-01-acting-does-not-drive-content-actions.md); board settled and decomposed 2026-08-15 into [FEAT-PD019](../../../platform/domain/features/FEAT-PD019-wielded-content-authorship-acting-in-content-contracts.md) + [FEAT-PD020](../../../platform/domain/features/FEAT-PD020-group-addressed-notification-delivery.md) (platform) and [FEAT-H046](./FEAT-H046-wielded-content-affordances.md) (the surface half). This spec stays 6-done as shipped — the "use" half lives there.
