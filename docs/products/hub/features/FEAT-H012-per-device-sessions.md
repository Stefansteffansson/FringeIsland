...
**Sibling sweep — found, deliberately NOT changed:** `AuthContext.signOut()` (`:243`) is also bare, so a deliberate "Sign out" ends every device. That is a **product-semantics question, not this defect**, and `profile.spec` STORY-4 currently depends on the global scope; raised for Stefan rather than changed. The Mist-erasure `signOut()` (`:238`) is per-device-anonymous, so scope is moot there.
