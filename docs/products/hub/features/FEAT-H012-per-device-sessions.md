...
### Amendment — W-05: the guard signed a member out of every device on a network hiccup (2026-07-27)

**Found in the A-NTF live walk (2026-07-27); out of that area, so it was escalated and fixed on its own.** Two defects in `validateSession`, one masking the other. **The bullet above is corrected in place by this note:** it claimed *"network failure is never treated as refusal"* — that was the written intent, and the guard's `catch` was written to honour it, but the code did the opposite.

- **The `catch` was dead code.** `supabase.auth.getUser()` **returns** its network error rather than throwing — `AuthRetryableFetchError` extends `AuthError`, and auth-js resolves the promise for anything `isAuthError` (it rethrows only non-auth errors). So a transient blip never reached the `catch`; it took `if (error)` and was handled exactly like a revocation.
- **The blast radius was global.** `signOut()` defaults to `scope: 'global'`, so that one device's hiccup ended **every** session the member had, on every device — silently, landing them on *"Welcome Back — Sign in to continue your journey."*

**Fix.** A new exported `isSessionRefusal()` classifies the result as a **whitelist**: only a positive refusal signs out — `isAuthApiError` with status **401/403**, or `AuthSessionMissingError`. A retryable fetch error, a 5xx, an unrecognised shape, and a genuine throw are all **inconclusive** and leave the session untouched (the fallback poll means an inconclusive check costs latency, never security — a blacklist would fail open on the next unfamiliar error shape; this fails closed). The sign-out is now `{ scope: 'local' }` — this device only, matching what the STORY-3 test's own name always claimed and the convention `app/farewell/page.tsx` already used. The inconclusive branch **emits `sessions.guard_validate_inconclusive`** rather than swallowing silently: the silent swallow is what let this hide behind a dead `catch`.

**Why it escaped.** Every existing guard test mocked `getUser` with a plain `{ error: { message: 'session_not_found' } }` — a generic truthy object that cannot exercise classification. The harness was retrofitted to real typed `AuthApiError`s, so the refusal path is now proven against the shape production actually returns.

**Red-first, both tiers.** Unit **5 new cases demonstrated red** (transient / 5xx / unrecognised shape / `scope: 'local'` / 401 + session-missing) → green; guard suite **8 → 13**, Hub unit **982 → 987/987** (131 suites). E2E `sessions.spec.ts` gained a fourth test that route-aborts `**/auth/v1/user**`, asserts the guard **actually revalidated** (so it cannot pass vacuously), and asserts no eviction — **demonstrated red by reverting the fix**, where one blip navigated the member to `/login`. Sessions E2E **3 → 4/4**. `next build` clean.

**Sibling sweep — found, raised, and since resolved:** `AuthContext.signOut()` (`:243`) was also bare, so a deliberate "Sign out" ended every device. That was a **product-semantics question, not this defect**, so it was raised rather than changed — and **ruled by Stefan the same day (2026-07-27): sign-out is local to the browser; a deliberate "Sign out everywhere" comes later.** See the amendment below.

### Amendment — sign-out is local to this browser (2026-07-27)

**Decision, not a defect.** `supabase.auth.signOut()` defaults to `scope: 'global'`, so signing out on a laptop also ended the member's phone and tablet, silently. That is the opposite of the near-universal convention — Google, Microsoft, Facebook, Apple, GitHub and Slack all treat "Sign out" as device-local and put *"sign out everywhere"* behind a separate, explicitly named control beside a device list. Signing out is routine (a shared machine, the end of a day); ending **every** session is a *security response*, a different intent, and should be asked for.

`AuthContext.signOut()` and the Mist-erasure drop (`:238`) now both pass `{ scope: 'local' }`, which makes **all five** sign-out call sites local and consistent: the session guard (W-05), `farewell`, the `/sessions` current-device revoke, the Mist farewell, and the account menu. Red-first: 2 unit cases demonstrated red (`Expected {"scope":"local"} / Received undefined`) → green; Hub unit 994 → **996/996**. Arming a FIM session in that suite for the first time also required realtime stubs — the existing cases only ever exercised sessionless and Mist.

**Correcting the note above:** it said `profile.spec` STORY-4 *depended* on the global scope. It does not — STORY-4 asserts only local effects (returns to `/`, `/groups` then gates). The real relationship was the reverse: because sign-out was global it **server-revoked the shared storageState session**, which is why `sessions.spec` had to switch to fresh logins. This change *removes* that suite hazard; the fresh-login pattern is kept deliberately, because that journey revokes sessions and must not revoke one another spec relies on.

**Still to come:** *"Sign out everywhere"* on `/sessions` is a **new capability**, not part of this change — it needs a platform bulk-revoke contract (today only `DELETE /api/sessions/[id]` exists), so it is a paired spec.
