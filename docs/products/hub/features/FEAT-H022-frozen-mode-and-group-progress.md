...
**Post-6-done note (same day, Stefan's live walk):** the sharing checkbox could repaint the *opposite* of the true state on a client-side revisit — intermittently, in both directions — while the leader panel stayed correct (it fetches fresh on every expand). Root cause, two halves of one staleness bug: `setProgressSharing` wrote nothing to the per-enrolment session cache (the "page owns optimistic progress" doctrine over-applied — sharing is server-confirmed state, not optimistic progress), and the toggle latched `useState(initialSharing)` at mount, ignoring the page's revalidated truth. The display trailed reality by one visit. Fixed red-first (2 reds → green; unit 656 → **660/660**): the mutation now **merges its server-confirmed response into the cached entry** (the completion-moment precedent — never the optimistic value; a failed write never touches the cache, pinned), and the toggle is **prop-driven with a last-confirmed override** (the page's truth renders unless this mount holds a fresher fact — an in-flight optimistic flip or the confirmed response; rollback falls back to the prop). The E2E arc gained the exact repro: **client-side revisits via history pops** (`goBack`/`goForward`) — the arc's original revisit used full page loads, which reset the module cache and masked the bug. Lesson banked for the retro: *a full-load revisit is not a client-side revisit* — E2E revisit assertions must stay in one JS context to witness cache staleness.
