...
**Post-6-done fix (2026-09-05, the Ferd close — [TASK-RACE-01](../../../planning/backlog/tasks/TASK-RACE-01-superseded-read-overwrites-current-view.md)):** `GroupForumSection.load` could be overwritten by a superseded read (a personal read still in flight when the hat went on, resolving last as a 403 and flipping the section to "the hat doesn't open this forum") — the shape that failed the H047 E2E journey on the test project. `load` now takes a read sequence number before its `await`; a superseded read never writes. Red-first unit cell in `GroupForumSection.acting.test.tsx`.
