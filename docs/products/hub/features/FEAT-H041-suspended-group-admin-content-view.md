...
**Amendment 2026-09-02 (TASK-SEAL-01, Hub half — sealed-thread sight).** The admin group page gains a sibling of this wing for a different state: a **closed** engagement group mounts `AdminClosedThreadsSection` ("Preserved threads"), fed by `GET /api/admin/groups/[id]/closed-threads` over the platform's `admin_get_group_conversations` (migration `20260811220000`, AB-6 ruling B1 as re-scoped to `closed` — the one state a sealed thread exists in, since `sealed_at` has a single writer that fires only while a group closes). Every group-kind thread is listed with its message count; a sealed thread wears a **"Sealed <date>"** badge and carries **no open affordance** — labelled, never presented as live (bound 3); direct conversations never appear (bound 2); the read is recorded durably (bound 4). This wing (suspended) and that section (closed) are mutually exclusive by state; the contracts refuse everyone else anyway. **Stated on the surface and here:** thread contents are not readable from the admin plane — no message-level admin contract exists; the member detail door keeps its `sealed_at IS NULL` law. A paired platform rider would be needed for that (schema gate); not part of this amendment. Pinned: unit (route, lib, section, detail mount) and E2E `admin-closed-threads.spec.ts`, red-first.

- **Two tranches around the schema gate** (the H039/H040 pattern): tranche 1 (#419, merged pre-apply) — the wing (`AdminSuspendedContentWing`: banner + members + `ForumPane`/`AnnouncementsPane`/`ConversationsPane` as colocated lean components), the six BFF routes, `lib/admin/content.ts`, both ceremonies; tranche 2 (post-apply of #418) — integration + the E2E journey. Red-first at the unit tier: the wing suite (12) and the route-tier suite (17) demonstrated red module-absent before implementation; the mount-gate cells in the H035 suite landed with the mount edit and are labelled as such; two H035 cells carry labelled adaptations (the wing's fetch-on-mount reads broke exact fetch-count assertions).
- **Named deviations from the solution sketch** (both verification-driven): the remove route segment is **`[userId]`**, not `[memberGroupId]` — the PC021 contract is keyed by `public.users.id`, served by PC026's `user_id` payload key (the payload-walk build finding); and `ConfirmModal` gained an additive optional **`confirmDisabled`** prop (the H039 message-widening's sibling) so the required-reason field inside the ceremony message gates Confirm. The remove route reuses the shared `removeAdminUserFromGroup` wrapper and the existing `admin.member_remove_from_group` durable event — one act, one door, one event.
- **The remove reason is ceremony-side deliberateness only** — PC021's signature carries no reason parameter (verified at build) and reasons never enter telemetry; the moderate reason travels to the platform audit row via the PC026 wrapper.
- **The route-tier suite pattern is new** (`hub/tests/unit/app/api/admin-group-content-routes.test.ts`, node-env with mocked server/wrapper/telemetry modules): no BFF route-level suite existed before this cycle — the anatomy sweep flagged that durable-telemetry emission was asserted nowhere — so STORY-6's "asserted in the route suites" is satisfied at this new tier (exact event names, exact ids-only prop sets, no-emit on refusals). Worth reusing on future admin routes.
- **State drift over zombie affordances:** a section refusal (404/403/401) signals `onStateDrift` → the parent re-reads the detail and the wing collapses if the status changed; the section itself settles into the honest error state (never an eternal skeleton, never a retry loop). This also made the pre-apply interim honest: banner + members without the new keys + three errored sections, admin-plane only, resolved at apply.
- **E2E journey 6/6** (`admin-suspended-content.spec.ts`), leak instrument 0→0 — suspend → the wing serves all four families → the member 404-refused on every new route while held → moderate (reason-gated, tombstone in the wing) → remove (email echo, row leaves) → `/admin/audit` carries both act rows → reactivate → wing gone, member plane restored (tombstone visible to the remaining member; the removed member gets the no-leak `Group not found`). Member-side verification is sequenced AFTER reactivation — while the hold stands, the member plane is quarantined by design (H038). Content fixtures are laid through the member contracts (labelled in the spec header); the journey itself is UI-driven end to end. Two failures during bring-up were test-side selector issues against CORRECT product behaviour (the busy modal's W-4 email echo matching the row locator; the member-plane no-leak copy differing from the admin-plane 404 copy) — both labelled in the spec.
- **Performance (ADR-U043):** first paint is unchanged — the wing renders below the already-painted metadata anatomy and adds no request to it; the sections are independent post-paint fetch-on-mount reads (justified standalone reads, ADR-U042 guardrail 3 — the admin plane has no overview bundle) with B6 skeletons and B5 ceremony feedback. No deep-cold spot measurement owed this cycle (no first-paint request added or rerouted); the standing admin-plane deep-cold item rides AB-6's full pass per the budget row above.
- Gates: unit 1291/1291 · lint 0 errors · `next build` green · route-policy + outer-ring conformance zero exceptions (per-verb identity convention matched: GETs `getVerifiedUserId`, POSTs `getUser()`).

**Amendment 2026-09-03 (TASK-SEAL-02, Hub half — the sealed thread, readable).** The preserved-threads section's rows gain exactly one affordance — **Open** — leading to a read-only thread view inside the section: the label *Sealed <date> — preserved when the group closed; nothing here is live*, every message oldest-first with the ladder-resolved sender (a departed author reads "Former member"), a removed message as a tombstone, no composer, no reply, no reactions; Back returns to the list. Fed by `GET /api/admin/groups/[id]/closed-threads/[conversationId]` over the platform's `admin_get_group_conversation_detail` (migration `20260903110000`; the SEAL-01 shape — wall, closed scope, DM no-leak, and the audit row, all platform-side): the admin-plane 404 collapse and durable telemetry `admin.sealed_thread_read` (ids only, never content). The SEAL-01 sentence "thread contents are not readable from the admin plane" is **retired** — from the section copy and from the record above; the member doors stay untouched. Red-first: unit — the route (3 cells), the lib wrapper (2), the section (5) — red → green; E2E `admin-closed-threads.spec.ts` extended (Open → the label + the evidence body → no composer → the audit row → Back), its SEAL-01 "never a door" assertion adapted and labelled (the row now has one button and still no link).
