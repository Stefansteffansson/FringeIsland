...
*(6-done, 2026-07-24 — built Cycle N-B, red-first. The solution sketch below stands as built, with the deviations named here.)*

- **Where it lives:** `hub/lib/notifications/format.ts` (the `notificationResponses` registry keyed by `action_type`, `isActionable`, the convergence chip, `firstToken`), `client.ts` (`respondToNotification` + `notificationDispatchRoute`), `hub/components/notifications/NotificationActions.tsx` (the generic affordance), `NotificationItem.tsx` (status chip + response window), `NotificationBell.tsx` and `hub/app/notifications/page.tsx` (the two response surfaces), `hub/app/api/notifications/[id]/acting-response/route.ts` (the new BFF route) with the `respondToActingInvitationRpc` courier.
- **Deviation from the sketch (structural, deliberate):** `NotificationActions` renders as a **sibling** of `NotificationItem`, not inside it. The bell wraps each row in a `<button>`, and buttons must not nest; both surfaces mount the affordance beside the shared row body instead. The "appears in both bell and inbox" property is preserved by mounting at both call sites.
- **Two acceptance criteria were unmet at the E2E phase and closed here, red-first (found during the close, not planned scope):**
  - **STORY-1 AC3 (the failure reason).** Both `respond` handlers rolled back on a failed dispatch but swallowed the reason in a bare `catch {}` — a silent rollback is indistinguishable from "nothing happened", which is exactly the divergence the criterion forbids. Both surfaces now pin the reason to its row (`notification-action-error-<id>`, `role="alert"`) and clear it on retry. Red demonstrated on both (element absent), then green.
  - **STORY-2 AC1 (the response window).** The retired `PendingNominations` section showed the nominee a concrete "Respond by <date>" from `expires_at`; the fold lost it entirely (`Respond by` existed nowhere in the tree). `NotificationItem` now renders it for rows still awaiting an answer, and drops it once answered or past the deadline (the chip states where it stands). Red demonstrated on the positive assertion; the three absence-guards passed at red by nature and gained teeth once the element existed — labelled honestly rather than claimed as four reds.
- **Red → green:** 940/940 unit green (934 inherited + 6 added: 1 bell, 1 inbox, 4 `NotificationItem`). `next build` green — the house type gate. The unit tier carries the component/logic behaviour; E2E carries the journeys.
- **E2E (three specs, all green):** the new `hub/tests/e2e/notification-actions.spec.ts` drives the genuinely-new N-B behaviour — a context group invites an engagement group whose **two** `act_as_group` holders each receive a fanned `acting_invitation`, one answers, and the co-leader's letter (re-read from the server) reads **"Answered by [name]"** with no buttons. Two existing specs were adapted to the fold rather than weakened: `group-of-groups.spec.ts` (the acting answer moved from the panel to the bell; the panel asserted read-only both ways — pointer present, buttons absent) and `leadership-transfer.spec.ts` (**both** nomination journeys, accept and decline, moved to the bell; `pending-nominations` asserted gone). The bridge flagged only the first of these as red — `leadership-transfer` was a second, unflagged red found by sweeping for the retired testids.
- **A real test bug found and fixed, not worked around:** the first adapted `group-of-groups` run left the membership `invited`. Navigating immediately after the confirm click **aborts the in-flight POST** — the optimistic paint hides it, so asserting "buttons vanished" proves nothing. All three specs now assert the dispatch itself (`waitForResponse` on the response route + status 200) before asserting the consequence. This is a test-timing artifact rather than a product defect: a real user who navigates mid-flight simply doesn't answer, and the next load re-reads server state honestly.
- **A moved assertion must keep its old scope.** `leadership-transfer`'s "the Surface never names the routing (next nominee vs FringeIsland)" check was scoped to the retired `pending-nominations` panel; re-homing it naively to the whole bell dropdown made it wrong, because the dropdown legitimately carries an unrelated `"Member" role in "FringeIsland Members"` notice. Re-scoped to the nomination's own row, which is what the original assertion meant. Caught because the retry loop stops on assertion failures instead of retrying through them.
- **Environment, fenced (found not caused):** the TASK-INT-01 GoTrue `unrecognized JWT kid <nil> for algorithm ES256` flake was actively degrading through this session and repeatedly killed `createFim` fixtures — control-proven environment, unchanged by this branch. Green was reached with the house auto-retry technique, run **per spec** (each spec its own invocation and retries) because a combined run rarely got one clean auth window; assertions are untouched, only the process boundary moved. The retry guard stops on any real assertion failure so the loop can never manufacture a green. Separately, `npx tsc --noEmit` reports ~1241 pre-existing errors repo-wide (jest-dom matcher types absent from the root tsconfig) in files this branch never touched — `next build` is the house type gate and is green.
- **Plain-English walkthrough (done at close; the narrative is the CHANGELOG entry).** Walked against shipped behaviour, two lifecycle cases the test tiers don't ask about came out **correct but worth naming**:
  - *A holder who loses `act_as_group` between the fan-out and their answer* still sees the buttons (their row is unresolved), but the untouched Core gate refuses `42501`. Before this cycle's STORY-1 AC3 fix that would have been a silent rollback; it now says why. This is the criterion earning its keep, not a hypothetical.
  - *A leader added to the invited group **after** the invitation was sent gets no letter* — fan-out is send-time by design (ADR-U049), and the notification set is fixed at invite time. Correct per the doctrine, but it means "everyone who can answer for that group" is true as of the moment of invitation, not continuously. Named here so it isn't rediscovered as a bug.
- **Named loose end (not closed here):** N-B deleted `PendingNominations`, the only consumer of the `/api/me/overview` bundle's `nominations` slice. The bundle still computes `get_my_pending_nominations` on every `/groups` first paint and ships a slice nothing reads (`fetchMyNominations` has no production caller left). Dead weight on a first-paint read rather than a defect — scoped as follow-up, deliberately out of this diff.
