...
**Amendment 2026-09-02 (TASK-MIST-01 — the ghost window, J-O3 area-gate finding).** A Mist erased server-side (the 72 h ADR-U033 reaper, or a goodbye said on another domain) while a browser still holds its session reads `identity === 'mist'` locally — the JWT is still signed, and the local derivation is correct by design (ADR-U037). Its first actor-bound read then refuses with `42501` "no resolvable actor", which the BFF now **names** with `code: 'no_resolvable_actor'` on `/api/me/onboarding` and `/api/me/journeys`. The arrival check (`OnboardingArrival`) and the Mist page's walk resolution treat that code as a broken session rather than a transient: `dropGhostSession()` (AuthContext; local scope — a Mist is per-device) ends the session, the visitor lands on the sessionless entry, and the next "Look around" mints a fresh Mist — the first-time experience restored within one interaction. Telemetry `mist.ghost_session_dropped`. **The per-domain session split, stated:** a browser session is per origin; a goodbye said on one domain (or an erasure by the reaper) cannot clear another domain's remembered Mist — that other browser discovers the erasure exactly this way, on its next actor-bound read. Pinned: unit (arrival, Mist page, both BFF routes, both clients), and E2E `mist-ghost.spec.ts` (mint → erase server-side → return → sessionless → fresh Mist), red-first at every tier.
