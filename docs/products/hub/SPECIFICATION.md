...
| [FEAT-H003](./features/FEAT-H003-mist-identity-on-arrival.md) — Mist identity on arrival: the FringeIsland entry and the lazy Mist actor | IDN-1 (anonymous Mist on arrival — sessionless public entry + lazy anon session + `is_temporary` + proto personal group + status gating); V2 privacy (data-min, unlinkable cross-session) · V4 telemetry. Carries a net-new migration (`users.is_temporary`, `handle_new_user` amendment, Visitor→Mist rename) | Ferd | 5-in-cycle | none |
