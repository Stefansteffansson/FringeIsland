...
- **Obligations must be tier-specific, not generic.** "Every service must log" is weak. "Each operation emits one structured log entry at the appropriate level carrying request ID, actor, and outcome; increments a count + duration metric under a canonical name; and writes an audit entry for every security-relevant action and every access to private member data" is enforceable. Rewrite any bullet that sounds like a slogan until it sounds like a checklist.
