...
- **Mist durability leak.** Notification state outlives the ephemeral session (ADR-U031): a Mist can hold no email address and no durable notification state — in-session delivery only. Detected by ephemerality verification on Mist-linked rows; recovered by the TTL/explicit-erase sweep. ***REALISED AND CLOSED, 2026-07-27 (A-NTF gate, NB-8).*** *This risk stopped being hypothetical: the first ephemerality verification ever run on Mist-linked rows — the very method named above — found **every** Mist holding a durable `role_assigned` row, written at signup by its own personal-group bootstrap and readable, mark-readable and exportable through the ordinary member doors. Not a cross-member leak (the row was self-referential and CASCADEd away on erase) but a straight violation of the rule this bullet states, and not Mist-specific either: 1516 of 1548 FIMs carried the identical row. **Closed by construction rather than by sweep** — migration `20260727180000` guards the bootstrap at source and makes the shared dispatcher refuse any row whose recipient is `is_temporary`, so the TTL/explicit-erase sweep is no longer the only thing standing between a Mist and durable state. [Proof](../../planning/hub-v2/2026-07-27-antf-nb8-mist-posture-proof.md).*
