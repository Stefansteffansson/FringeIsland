...
Every domain service can produce events users care about. This vertical defines the shared notification fabric — email, push, in-app — so that any service can publish a notification without reinventing delivery, preferences, or rate limiting. Notifications are how the platform reaches users when they aren't actively using it. Done right, they pull users back into meaningful moments. Done wrong, they're spam — and a one-way ticket out of the user's trust.
