...
- **Preferences, quiet hours, frequency caps, digest (all unrealized).** No `notification_preferences` store, no quiet-hours or caps machinery, no digest batching, no suppression list — to-be-designed; preferences storage is a Platform Core obligation (§6).
...
- Notification preferences are member state stored in Platform Core (PC-2): per-category, per-channel. Consent state stays authoritative in Core; surfaces and DS-5 read and present it, never own it.
