...
4. **Admin-role granularity.** Today's model is one DeusEx role holding all permissions. Does universe-scoped administration differentiate seats (moderation vs lifecycle vs economy vs legal — the Universeers' five care domains suggest the axes)? The bible's roles and governance chapter carries the Universeers'-internal-structure question (consumed here, not resolved); the role-stripped divergence (§4) shows the cost of leaving granularity implicit. Candidate ADR alongside the parked root-admin principle.
5. **Breach-response process (V1's half of V2 §5 Q5).** Detection is V4's (landed); the 72-hour Art. 33/34 duty's process half — who declares an incident, who notifies, the runbook — is operator-plane work this vertical owns. Seams with V4 §5 Q3 (on-call posture: V4 owns escalation tooling; V1 owns who answers). Candidate joint spike with V2/V4. **Resolved (2026-06-13, breach-response joint-design spike — [design record](../../fringeisland-thinking/record--breach-response-gdpr-art-33-34.md)):** the incident is **declared by the legal/governance care-domain seat** (a role, not a person — ADR-U028 role-based authority; pre-launch Stefan holds the seat). The handoff: a V4 breach-candidate signal -> this vertical's **triage** (initial assessment) -> **awareness declared = the 72-hour clock starts**, with both signal-time and awareness-time stamped into the breach register (the anti-gaming control). This vertical owns the **runbook**: declaration, the notification acts, the Art. 33(3) authority-notice template, and the phased-notification posture (notify on time with what is known, supplement later). On-call: V4 owns escalation tooling, this vertical owns who answers (V4 §5 Q3 seam).
