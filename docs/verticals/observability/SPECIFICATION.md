...
status: active
last_updated: 2026-07-31  # ADR-U052 reconciliation (Cycle ADM-A): sink realized, §3/§5 statuses updated
tier: Cross-cutting
---
...
- **Metrics/event backend (SELECTED for Ferd — ADR-U052, 2026-07-31).** The sink is the PC-1-owned Postgres event store `telemetry_events` (FEAT-PC018): durable, deny-all under RLS, written by the never-raises `record_telemetry_event()` recorder, 90-day raw retention via the `telemetry-prune` pg_cron job, aggregates computed on read. **No external vendor in Ferd** (zero consumers a vendor would serve; a drain is re-openable at Eid without migration cost). The first dashboard is realized: the Hub's `/admin` (ADM-1, FEAT-H034) renders live counts + a 30-day activity trend from the admin-gated `get_platform_statistics()`. Count+duration metrics under canonical names and alerting remain forward (the event stream carries names + outcomes today, not durations).
...
1. **Self-host or buy observability infrastructure?** ~~Candidate decision when the first backend is selected (§3).~~ **ANSWERED for Ferd (ADR-U052 §1, 2026-07-31):** own Postgres event store, no external vendor — no sub-processor surface bought for zero consumers. Re-openable at Eid (the store is the source either way). The error-reporter half of the question stays open (§3).
2. **Retention per signal class.** **PARTIALLY ANSWERED (ADR-U052 §3/§4, 2026-07-31):** telemetry events — 90 days, mechanically pruned, legitimate-interest basis, erasure cascades with the personal group (proven by test); audit entries — indefinite, append-only, with the right-of-access split decided (§6 there: own-actor rows export, target-of-admin rows exempt narrowed). Still open: the runtime log stream's retention basis (Vercel default today) — the remaining V2 joint item.
3. **On-call during the solo-developer phase.** Who is paged, when, why — and what does "paged" mean for one operator? Interim posture candidate: a periodic review cadence plus a single escalation channel; becomes real when alerting tooling lands (§3; §4 recorded-but-unseen). **Advanced (2026-06-13, breach-response joint-design spike — [design record](../../fringeisland-thinking/record--breach-response-gdpr-art-33-34.md)):** for breach-candidate signals the now-scale obligation is committed — signals route to an **active alert channel the operator actually checks** (push/email) on a documented review cadence; logged-and-unseen is the §4 recorded-but-unseen failure. The residual risk is named honestly: a breach during an unavailable window can burn the 72-hour clock until alerting tooling lands. Later-scale: an escalation rotation tied to the legal/governance seat (V1 §5 Q5).
...
5. **Lawful basis for behavioral telemetry.** **ANSWERED (ADR-U052 §4, 2026-07-31):** operational telemetry and audit run under documented legitimate interest — content-free, actor-linked by identifier, retention-bounded; "analytics/optimization" is a registered ADR-U034 consent purpose whose collection stays dormant until a consumer exists (nothing behavioral is collected that nothing reads). The line between the two classes is content + purpose, reviewed per feature via the Vertical Impact slot.
...
- Server-side emissions have a durable path: the PC-1 sink (`telemetry_events` via `record_telemetry_event()`, ADR-U052) is the platform's event store — fire-and-forget (an emit failure never fails the emitting action), content-free, 90-day-bounded, erasure-cascading. The emit-without-store state is closed; a new server-side emitter that bypasses the recorder needs a stated reason. *(Added 2026-07-31, Cycle ADM-A — the ADR-U052 realization of this vertical's sink obligation.)*
...
- The **breach register** (Art. 33(5)) is a platform recording duty: every personal-data breach is recorded whether or not it was notified. It is distinct from `admin_audit_log` (admin-acts-only — a breach is an incident, not an admin act) and is an instance of the ADR-U012 record/expose split (this vertical records; V2 exposes). Each entry carries: incident ID; signal-time and awareness-time; declarer seat; affected data categories and sensitivity tier; approximate subject/record counts; compromise type; risk band with factor rationale; notification decisions with timestamps; measures taken; status; and the source V4 signal reference. (breach-response joint-design spike, 2026-06-13 — [design record](../../fringeisland-thinking/record--breach-response-gdpr-art-33-34.md).)
- **Breach-candidate signals** (RLS-denial / private-by-default inversion, payload leak, a retroactively-discovered audit gap, a processor-side report) must **escalate to a human**, not merely record — the escalation side of the §4 recorded-but-unseen failure and the Art. 33 clock-start seam. Escalation tooling is this vertical's; who answers is V1's (§5 Q3; V1 §5 Q5).
