...
- **Tier file (read first per load order):** [`../CLAUDE.md`](../CLAUDE.md) — every verticals-tier rule applies here without restatement. The tier file's canonical example for tier-specific obligation shape ("each operation emits one structured log entry at the appropriate level carrying request ID, actor, and outcome; increments a count + duration metric under a canonical name; writes an audit entry for every security-relevant action and every access to private member data") is drawn from this vertical's `SPECIFICATION.md` §6 because Observability is the cleanest illustration of the generic tier-obligation pattern, not because it carries entity-specific authoring discipline.
