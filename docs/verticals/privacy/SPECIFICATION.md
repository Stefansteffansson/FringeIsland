...
- The service owning the FIM's private home enforces granular sharing (universe-discovery S43; see `../../fringeisland-thinking/canon--growth--privacy-model.md`): per-region, per-audience, and revocable — the FIM can open one room to one audience and keep the rest locked, and the FIM holds the only key by default
