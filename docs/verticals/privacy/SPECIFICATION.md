...
- Right to rectification (correct) — GDPR Art. 16
...
- **Rectification (Art. 16) — partial, admin-only.** The login identifier is correctable as of 2026-09-11: [ADR-U054](../../architecture/decisions/ADR-U054-admin-email-rectification.md) with [FEAT-PC031](../../platform/core/features/FEAT-PC031-member-email-rectification-contract.md) and [FEAT-H050](../../products/hub/features/FEAT-H050-admin-member-email-rectification.md). What exists is one administrator-initiated contract over one identifier, not a rectification pipeline. **Self-service rectification does not exist** and is gated on outbound mail the platform does not have (§5 Q8). There is no general "correct this field" path, and no member-initiated correction request flow. Before that ADR, the platform could not honour an Art. 16 request at all — the only remedy was erasure and re-registration, which is not rectification.
...
- **Rectification shortfall.** A correction lands in one store and not the others, so the platform holds two answers to the same question — or a correction succeeds but strands data keyed to the *old* value beyond the reach of a later Art. 17 request, which converts a rectification into a quiet erasure failure. Detected by asserting every store in one transaction and by proving the erasure path still reaches what the correction left behind; structural wherever a value is copied without a correction path. Recovered by completing the correction across every store and re-running the erasure check. The realized instance is the login identifier: three stores plus the invitation table, corrected atomically by `admin_update_user_email`.
...
5. **Breach response (Art. 33/34).** The 72-hour notification duty exists from the first byte of personal data; no detection-to-notification process is defined. Candidate spike; seams with V1 Administration (process) and V4 Observability (detection). **Resolved (2026-06-13, breach-response joint-design spike — [design record](../../research/breach-response-design.md)):** the end-to-end process is designed — detect (V4) -> assess -> clock -> notify authority -> notify members -> record. This vertical owns the **risk-grading rubric** (three bands — unlikely-to-risk / risk / high-risk — over six factors, with a FringeIsland Tier-A self-reflection-content clause biasing toward member notification) and the **member-notice content rules** (the Art. 34 floor reconciled with V3's content-minimisation ceiling: the required elements, no more). V2 **exposes** the breach register that V4 records (the ADR-U012 split). Processor-side breaches (Art. 33(2); Stripe is a named sub-processor): the contract must guarantee prompt notification with sufficient detail, and the platform's clock starts at receipt of the processor's report — this feeds the sub-processor rule. Lead supervisory authority = Integritetsskyddsmyndigheten (IMY), Sweden's DPA, on a provisional Sweden-establishment assumption (revisit if establishment changes; the obligation to confirm before scaling EU personal-data processing stands). The DPO/contact-point question stays open at §5 Q1.
...
8. **Self-service rectification, and the mailer it waits on.** A member cannot correct their own identifier; only an administrator can, and the member is told in-app because the platform has no outbound mail of its own (ADR-U054 accepted this as a limitation, not a design preference). Two questions follow: does Art. 16 oblige a *self-service* path, or is a responded-to request sufficient; and what is the notification duty to the **old** address, which is the natural preventive control against an administrator redirecting an account and is unavailable today. Both unblock on the same Supabase plan decision. Candidate ADR when outbound mail exists.
9. **Rectification against AI-derived state.** §5 Q2 asks where erasure stands against data embedded in model weights and vector indexes. Art. 16 has the same problem and is not covered by Q2's framing: a corrected fact does not propagate into an embedding computed from the stale one. Qualify, never promise — the same discipline the erasure gotcha imposes.
...
- **An inaccurate personal datum is correctable, and the correction is atomic across every store that holds it (Art. 16).** Where a value is mirrored — and the login identifier is, across `auth.users`, the `auth.identities` provider record and the `public.users` mirror — one transaction writes all of them or the platform holds two answers to the same question. A correction must also leave the erasure path still able to reach whatever was keyed to the **old** value; data stranded by a correction is an Art. 17 failure wearing an Art. 16 coat. A value that is copied without a correction path is a defect at the point of copying, not at the point of correction
- **Correcting an authentication identifier is a credential event, not a profile edit.** It ends the member's sessions, and it is audited with both the previous and the new value — the previous one is the only record that the account was ever reachable there, and its retention is a deliberate decision, not residue
...
- **A service does not copy a member-supplied identifier it cannot correct (Art. 16).** Either it references the owning store, or it carries a correction path invoked by the same transaction that corrects the source. No Domain Service holds a member email today — the only copy outside Platform Core's own stores is `pending_email_invitations.invited_email` (PC-3), which holds an address for someone who may not be a member at all and therefore also engages §5 Q7. This obligation is forward law: it exists so the next service that wants to denormalise an identifier meets the rule before the copy is made, not after
...
- Export, erasure **and correction** request flows are product-tier UI deliverables, even though the data and pipelines live in Platform Core
- **A correction surface names its consequences before the act, not after** — what ends (sessions), what is removed (anything addressed to the old value), and what the member will *not* receive. Where the platform cannot notify through the channel a member would expect, the surface says so rather than letting the operator assume it happened
...
- [ ] New personal data field that can be wrong has a correction path (GDPR Art. 16), or a written reason it cannot be corrected; correction against AI-derived state is an open question — never promised (§5 Q9)
- [ ] New copy of a member-supplied identifier is corrected by the same transaction that corrects its source — and the erasure path still reaches whatever the correction leaves behind
...
- **2026-09-11 (Art. 16 amendment).** Rectification was absent from this spec: §2 enumerated access and erasure and omitted correction, and §6 levied no correction obligation on any tier. The gap was found while investigating whether a member's email address could be changed — it could not, by anyone, and the only remedy was erasure and re-registration, which is not rectification. Closed by amendment rather than re-derivation: §2 gains the right, §3 records the partial and admin-only tooling, §4 gains the rectification-shortfall failure mode, §5 gains Q8 (self-service, gated on outbound mail) and Q9 (correction against AI-derived state, the Art. 16 twin of Q2), §6 gains one obligation per tier, and §7 gains two checklist items. Authority: [ADR-U054](../../architecture/decisions/ADR-U054-admin-email-rectification.md), realized by FEAT-PC031 / FEAT-H050 (`6-done`, 2026-09-11). **Grounding for the Domain Services obligation:** a live-schema sweep of `information_schema.columns` across `public` and `auth` for identifier-shaped columns found member email in exactly three places — `auth.users.email`, `auth.identities.email` (generated) and `public.users.email` — plus `pending_email_invitations.invited_email`, which holds a non-member's address. No Domain Service table holds one, so that obligation is forward law, stated so the next denormalisation meets the rule before the copy is made. **DoD reach, checked rather than assumed:** PROCESS.md §5 line 201 reads *"Vertical checklists pass — the feature satisfies each applicable cross-cutting checklist in the five vertical specs (`docs/verticals/*/SPECIFICATION.md` §7)"*. It delegates **by reference**, not by enumeration, so the two new §7 items are already inside the Definition of Done and PROCESS.md needs no edit. (A first draft of this block claimed the opposite; the claim was wrong and is corrected here rather than quietly dropped.) **Not closed by this amendment:** the Privacy bullets in `docs/platform/CLAUDE.md` and `docs/products/CLAUDE.md` do not yet carry the correction obligation — the products one names export and deletion as product-tier UI responsibilities and omits correction, which is now drift. Both are steering-file edits and are tracked separately. The studios and design-system bullets are unaffected: the new obligations do not touch attribution privacy or component-level render state.
