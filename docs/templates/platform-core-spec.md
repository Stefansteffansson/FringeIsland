...
*L2 authorship. Derived from Vision, the platform-tier `CLAUDE.md`, and the ecosystem anatomy (`../../architecture/ECOSYSTEM_ANATOMY_V4.svg`, ADR-U023). Revised when the area's boundaries, contract surface, dependencies, or stability posture change. Changes here are rare by design — see §7.*
