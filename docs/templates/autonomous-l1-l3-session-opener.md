...
| Verticals close-out (tool-level catch class, fourth member) | 2026-06-12 | LANDED §6 **pathspec under-scoping** (V5: a migrations-scoped probe false-zeroed on the seeds-resident permission catalog; judge every zero's scope against where the artifact class lives). Provenance: Transactions closing bridge (`2026-06-12_06_-_TRANSACTIONS-LANDED.md`). |
| Verticals close-out (series verdict addendum) | 2026-06-12 | LANDED §13 **verticals-series retraction verdict** (V2 1; V4/V1/V3/V5 0; the §5a disk-anchor discipline closed the realization-assertion class — held at both post-promotion traps). Provenance: V2-V5 closing bridges + the verticals close-out adjudication. |
| V1 Administration close (eleventh instance; third vertical) | 2026-06-12 | LANDED §5a **Step-1-realization-claims-need-disk-anchor** (n=3: V2 Privacy retraction as the without-case — realization asserted from ADR-U004 prose; V4 Observability decisive prevention — the archived-migration immutability prose, exactly the banked class; V1 clean hold at the pre-named ADR-U014 feature-flags trap, zero retractions). Realization claims cite a disk anchor (opener section-5b calibration block) or are explicitly lock-only; the Step 1 checkpoint carries a rider-compliance statement. Provenance: V2 + V4 + V1 closing bridges. |
| Phase 3 close-out (tool-level catch class) | 2026-06-12 | LANDED §6 **tool-level empty-result catch class** (three instances across two runs: the sandbox grep `-o` false-empty at DS-7 + the Extension System RPC survey; the piped-exit-code artifact — judge zero-hit claims by output lines, never `$?`). Provenance: DS-7 + Extension System closing bridges. |
...
**Realization claims need a disk anchor (standing; promoted at the V1 Administration close).** At Step 1, never assert that substrate is *realized* from prose — ADR decision text, migration comments, and prior-bridge characterizations are all prose (ADR-U004's "a pg_cron job cleans up" and the archived audit-log migration's "immutable... No UPDATE or DELETE for anyone" are the canonical traps). Every Step 1 realization claim carries a disk anchor — the opener's §5b calibration block is the sanctioned source — or is explicitly marked lock-only. The Step 1 checkpoint includes a rider-compliance statement enumerating each realization claim with its anchor; any trap the opener pre-names is dispositioned explicitly. Evidence: V2 Privacy (the without-case — one retraction from ADR prose), V4 Observability (decisive prevention at the banked class), V1 Administration (clean hold at the pre-named ADR-U014 trap; zero retractions).
...
- **Empty-result verification.** Every empty listing or zero-hit enumeration that feeds a claim is verified by a **second, differently-shaped method** (grep vs ls vs explicit-path test; loose-pattern vs word-boundary) before the claim is made. A silently-failing `find` produced false-empty listings three times on 2026-06-10 (DS-3 Step 2 + twice in the post-DS-3 doc-health run); within the DS-4 run the second method twice caught what the first missed (a loose-pattern `media` false positive inside "intermediate", exposed only by method contrast; an `avatars` migration hit the bucket-pattern sweep missed). The contrast between methods is itself evidence — record what each shape found. (n=4; landed after DS-4.) **Tool-level catch class (three instances across two runs):** the sandbox grep's `-o` flag silently returned empty against known-present content twice (DS-7; the Extension System's RPC survey) — method contrast caught both; and piped exit codes report the tail command's status, not grep's — judge zero-hit claims by output lines, never `$?`. (Landed at Phase 3 close-out.) **Pathspec under-scoping (fourth member):** a zero is only a zero for the scope probed — a `publish_journey` probe scoped to `supabase/migrations` returned zero while the live catalog sits in `supabase/seeds/` (V5). Before accepting any zero, check the probe's scope against where the artifact class actually lives; on a surprising zero, re-probe repo-wide. (Landed at the verticals close-out.)
