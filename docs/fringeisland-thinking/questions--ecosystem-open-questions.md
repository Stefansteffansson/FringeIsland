...
**Blocked by:** the universe-mechanics fundamentals (how the near and far ends of place 2 and place 3 actually look and function) — tracked in the [discovery backlog](discovery--tracker-and-backlog.md#discovery-backlog-open-topics-awaiting-sessions).
...
**Resolution path:** a **discovery session** (the mechanism that produced the universe canon — see [`universe-discovery/`](discovery--tracker-and-backlog.md)) to set the intended Hub experiential trajectory and its wave-staging, then reconcile the DESCRIPTION / §L3 / ROADMAP. Honors fundamentals-before-experience-design. Source: finding F-07 in [`../planning/hub-v2/phase-1-review-findings.md`](../planning/hub-v2/phase-1-review-findings.md). Related: CQ-010 (first hour), CQ-012 (Whisp representation), CQ-014 (visitor experience), CQ-015 (Hub greenfield rebuild).
